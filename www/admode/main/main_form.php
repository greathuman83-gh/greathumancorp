...
?>
<form name="fwrite" method="post" action="./main_ok.php?<?=$funcLibrary->queryString()?>" onsubmit="return fwrite_submit(this);" enctype="multipart/form-data" style="margin:0px;">
...
<tr class="ht">
	...
	<td class="td2">
		<div class="file_list">
			<input type="file" class="input_text" name="file1" onchange="imgFileCheck(this,<?=IMG_SIZE?>)"> <?=${'size_'.$pageType}?>
			<?php if($d['file1']){?>
				<input type="hidden" name="old_file1" value="<?=$d['file1']?>">
				<br>
				<img src="<?=$ghPath?>data/main/<?=$d['file1']?>" width="150" style="margin-top:5px;">
	...
	<td class="td2">
		<div class="file_list">
			...
			<?php if($d['file2']){?>
				...
				<br>
				<img src="<?=$ghPath?>data/main/<?=$d['file2']?>" width="100" style="margin-top:5px;">
...
<table width="100%" cellpadding="0" cellspacing="0" class="adminMenuTable" style="margin-top:30px;">
	<tr>
		<td align="center">
			...
			<button type="button" class="gray_btn" onclick="javascript:window.location='./main_list.php?<?=$funcLibrary->queryString('idx,w')?>'">취 소</button>
		</td>
	</tr>
</table>
</form>
