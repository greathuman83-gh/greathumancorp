...
<!-- 컨텐츠 -->
<form name="fwrite" method="post" action="./popup_ok.php?<?=$func_library->queryString()?>" onsubmit="return fwrite_submit(this);" enctype="multipart/form-data" style="margin:0px;">
...
<tr class='ht'>
	...
	<td class="td2">
		...
		<?php if($d['file1']){ ?>
			...
			<br><br><img src="<?=$gh_path?>data/<?=$upload_directory?>/<?=$d['file1']?>" width="150">
...
<table width="100%" cellpadding="0" cellspacing="0" class="adminMenuTable" style="margin-top:30px;">
	<tr>
		<td align="center">
			...
			<button type="button" class="gray_btn" onclick="javascript:window.location='popup_list.php?<?=$func_library->queryString('idx,w')?>'">취 소</button>
		</td>
	</tr>
</table>
</form>
...
{
	var popSubject = document.getElementById('pop_subject');
	if(popSubject.value == ''){
		alert('타이틀을 입력해 주세요.');
		return false;
	}
