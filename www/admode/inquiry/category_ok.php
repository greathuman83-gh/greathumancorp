...
if($w == 'a'){
	...
	if($depth == 1){
		...
		$c_code = substr($d['c_code'],$count,3)+1;
		$c_code = sprintf("%03d", $c_code);
		$c_code = $ccode.$c_code;
	...
	if(!$DB->insertInto($table_name, $inputs)){
		...
		$func_library->alert('등록되었습니다.',"category_list.php?menu_code=$menu_code&cate=$cate#$c_code");
	}
	...
	if(!$DB->updateSet($table_name, $inputs, $where)){
		...
		$func_library->alert('수정되었습니다.',"category_list.php?menu_code=$menu_code&cate=$cate");
	}
	...
	if(!$DB->delete_db($table_name,$where)){
		...
		$func_library->alert('삭제 되었습니다.',"category_list.php?menu_code=$menu_code&cate=$cate");
	}
}
