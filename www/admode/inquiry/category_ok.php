...
if($w == 'a'){
	...
	if(!$DB->insertInto($tableName, $inputs)){
		...
		$funcLibrary->alert('등록되었습니다.',"category_list.php?menuCode=$menuCode&cate=$cate#$c_code");
	}
	...
	if(!$DB->updateSet($tableName, $inputs, $where)){
		...
		$funcLibrary->alert('수정되었습니다.',"category_list.php?menuCode=$menuCode&cate=$cate");
	}
	...
	if(!$DB->delete_db($tableName,$where)){
		...
		$funcLibrary->alert('삭제 되었습니다.',"category_list.php?menuCode=$menuCode&cate=$cate");
	}
}
