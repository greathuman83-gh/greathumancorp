...
<tr class='bgcol1 bold col1 ht center'>
	...
	<td>상태</td>
	<td>등록일</td>
	<td><!-- <button type="button" class="red_btn" onclick="window.location='./report_form.php?<?=$func_library->queryString('pg')?>w=a'">등록</button> --></td>
</tr>
...
<?php
	...
	$func_library->appendWhereLike($where, $bind_param, $key_type, $keyword, 'r_name');
...
<tr class="list col1 ht center" height="30">
	<td><?=$number?></td>
	<td><a href="report_form.php?<?=$func_library->queryString('pg,idx,w')?>w=u&idx=<?=$d['idx']?>"><?=$d['title']?></a></td>
	<td><a href="report_form.php?<?=$func_library->queryString('pg,idx,w')?>w=u&idx=<?=$d['idx']?>"><?=$d['r_name']?></a></td>
	<td><?=$d['r_email']?></td>
	<td><?=$status?></td>
	<td><?=$regdate?></td>
	<td>
		<button type="button" class="black_icon_btn" onclick="location.href='./report_form.php?<?=$func_library->queryString('pg,idx,w')?>w=u&idx=<?=$d['idx']?>'">수정</button>
		<button type="button" class="gray_icon_btn" onclick="if(confirm('정말 삭제하시겠습니까?'))location.href='./report_ok.php?<?=$func_library->queryString('pg,idx,w')?>w=d&idx=<?=$d['idx']?>';">삭제</button>
	</td>
</tr>
<tr><td colspan="9" class='line2'></td></tr>
<?php $number--; }?>
...
<script type="text/javascript">
	function xls_down(){
		window.location = "./report_excel.php?<?=$func_library->queryString('pg,idx,w')?>";
	}
</script> -->
