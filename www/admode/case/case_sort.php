...
<table width="100%" cellpadding="0" cellspacing="0" class="adminMenuTable">
	<tr>
		<td>총 제품 수 : <?= $list_result['list_total']; ?></td>
	</tr>
</table>
<form name="fwrite" method="post" action="./case_sort_ok.php?<?= $func_library->queryString() ?>w=u&table_name=<?= $table_name ?>" onsubmit="return formSubmit(this);" enctype="multipart/form-data" style="margin:0px;">
	<table width="100%" cellpadding="0" cellspacing="0" class="adminMenuTable">
		<col width="500">
		</col>
		<col>
				...
				<div style="overflow-y:scroll;height:500px;">
					...
					?>
						<div class="caseList" <?= $_style ?>>
							...
							<ul>
								<li class="img"><?php if ($d['thumb_file']) { ?><img src="<?= $gh_path ?>data/case/<?= $d['thumb_file'] ?>" width="50"><?php } ?></li>
	...
	<p align="center" style="margin-top:30px;width:980px;">
		...
		<button type="button" class="gray_btn" onclick="javascript:window.location='./case_sort.php?<?= $func_library->queryString() ?>'">취 소</button>
	</p>
</form>
<br>
<script type="text/javascript">
	...
	// 사례 목록 순서 — 선택 후 upDown으로 DOM 재배치
	document.addEventListener('DOMContentLoaded', function () {
		document.addEventListener('click', function (e) {
			var item = e.target.closest('.caseList');
			if (item) {
				document.querySelectorAll('.caseList').forEach(function (el) {
			...
			var selected = document.querySelector('.caseList.on');
			if (!selected) {
				alert('제품을 선택해 주세요.');
				return;
			}

			var items = Array.prototype.slice.call(document.querySelectorAll('.caseList'));
