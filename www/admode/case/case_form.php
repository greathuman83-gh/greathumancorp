...
$size = '(477 x 278)';
$sizeDetail = '(514 x 300)';
?>
<form name="fwrite" method="post" action="./case_ok.php?<?= $funcLibrary->queryString() ?>" onsubmit="return fwrite_submit(this);" enctype="multipart/form-data" style="margin:0px;">
		...
		<tr class="ht">
			...
			<td class="td2">
				...
				<?php if ($d['thumb_file']) { ?>
					...
					<br>
					<img src="<?= $ghPath ?>data/case/<?= $d['thumb_file'] ?>" width="200" style="margin-top:5px;">
					...
					<?php for ($i = 0; $i < count((array)$attachFiles); $i++) { ?>
						<div class="fileList">
							...
							<?php if ($attachFiles[$i]) { ?>
								<br>
								<img src="<?= $ghPath ?>data/case/<?= $attachFiles[$i] ?>" width="200" style="margin-top:5px;">
	...
	<table width="100%" cellpadding="0" cellspacing="0" class="adminMenuTable" style="margin-top:30px;">
		<tr>
			<td align="center">
				...
				<button type="button" class="gray_btn" onclick="javascript:window.location='case_list.php?<?= $funcLibrary->queryString('idx,w') ?>'">취 소</button>
			</td>
		</tr>
	</table>
</form>
