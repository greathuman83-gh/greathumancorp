...
?>
<!--<button type="button" class="black_btn" onclick="window.location='./cert_sort.php?<?=$funcLibrary->queryString()?>'" style="margin-bottom:5px;">순서관리</button> -->
...
<tr class="bgcol1 bold col1 ht center">
	...
	<td>타이틀</td>
	<td>등록일</td>
	<td><button type="button" class="red_btn" onclick="window.location='./cert_form.php?<?=$funcLibrary->queryString('w')?>w=a'">등록</button></td>
</tr>
<?php
	$where = "where 1=1";
	...
	$thumbImg = $d['file1'] ? '<img src="'.$ghPath.'data/cert/'.$d['file1'].'" width="100" style="vertical-align:middle">' : '';
?>
<tr class="list col1 ht center">
	<td><?=$number?></td>
	<!-- <td>
		<form action="./cert_ok.php?<?=$funcLibrary->queryString()?>w=oe&idx=<?=$d['idx']?>" method="post">
	...
	<td>
		<a href="./cert_form.php?<?=$funcLibrary->queryString('pg,idx,w')?>w=u&idx=<?=$d['idx']?>"><?=$d['title']?></a>
	</td>
	<td><?=$regdate?></td>
	<td>
		<button type="button" class="black_icon_btn" onclick="window.location='./cert_form.php?<?=$funcLibrary->queryString()?>w=u&idx=<?=$d['idx']?>'">수정</button>
		<button type="button" class="gray_icon_btn" onclick="if(confirm('정말 삭제하시겠습니까?'))location.href='./cert_ok.php?<?=$funcLibrary->queryString()?>w=d&idx=<?=$d['idx']?>';">삭제</button>
	</td>
</tr>
