...
if($w != 'd'){
	...
	if($bbsid == 'recruit'){
		...
		for($i=0;$i<count((array)$b_data5);$i++){
			$recruitTypeArray .= $b_data5[$i].'|';
		}
		$recruitTypeArray = substr($recruitTypeArray,0,-1);
		$inputs['b_data5'] = $recruitTypeArray;
	}


	//================= 파일 첨부 시작 =========================
	if($del_file_thumb ??= null){
		@unlink($ghPath."data/board/$bbsid/$old_file_thumb");
		$inputs['file_thumb'] = '';
	}
	...
	if($_FILES['file_thumb'] ??= null){
		...
		if($file && $file_size>0){
			@unlink($ghPath."data/board/$bbsid/$old_file_thumb");
			$mfile = $funcLibrary->uploadFile('file_thumb','',$ghPath."data/board/$bbsid");
			...
			//$inputs['file_thumb'] = $resizeData['fileName'];
		}
	}
	...
	if($w != 're' && $boardInfo['b_file']){
		for ($i=0;$i<count((array)$_FILES['b_file']['name']);$i++){
			...
			if($b_file_name == ''){
				if (${'del_file'.$i} ??= null){
					@unlink($ghPath."data/board/$bbsid/".$o_name[$i] ??= '');
					$b_file_01[$i] = '';
					$b_file_01_name[$i] = '';
				...
				if($_FILES['b_file']['size'][$i] > 0){
					$mfile = $funcLibrary->uploadMultiFiles('b_file','',$ghPath."data/board/$bbsid",$i);
					@unlink($ghPath."data/board/$bbsid/".$o_name[$i]);
					if($mfile['filename']){
...
if($w == 'a'){
	...
	if(!$DB->updateSet("gh_board_$bbsid", $inputs, $where)){
		...
		$funcLibrary->alert('수정되었습니다.','./board_list.php?'.$funcLibrary->queryString('idx,w'));
	}
	...
	if (!$row['depth']){
		...
		$funcLibrary->alert('더 이상 답변을 달 수 없습니다.\\n\\n답변은 26개 까지만 가능합니다.');
	...
	$total = $queryLibrary->dataTotal($where,$bindParam,"gh_board_$bbsid");
	...
	if(!$DB->delete_db("gh_board_$bbsid",$where)){
		...
		for($i=0;$i<count((array)$b_file);$i++){
			@unlink($ghPath."data/board/$bbsid/".$b_file[$i]);
		}
		@unlink($ghPath."data/board/$bbsid/".$d['file_thumb']);
		$funcLibrary->alert('삭제 되었습니다.','./board_list.php?'.$funcLibrary->queryString('idx,w'));
	}
}
