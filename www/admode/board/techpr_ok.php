...
if($w == 'a'){
	...
	if(!$DB->insertInto($tableName, $inputs)){
		...
		$funcLibrary->alert('등록되었습니다.','./techpr_list.php?'.$funcLibrary->queryString('idx,w'));
	}
	...
	if(!$DB->updateSet($tableName,$inputs,$where)){
		...
		$funcLibrary->alert('수정되었습니다.','./techpr_form.php?'.$funcLibrary->queryString());
	}
	...
	if(!$DB->delete_db($tableName,$where)){
		...
		$funcLibrary->alert('삭제 되었습니다.','./techpr_list.php?'.$funcLibrary->queryString('idx,w'));
	}
}else if($w == 'oe'){//순서 변경
	$inputs = array();
	$inputs['num'] = $num;
	...
	if(!$DB->updateSet($tableName,$inputs,$where)){
		...
		$funcLibrary->alert('수정되었습니다.','./techpr_list.php?'.$funcLibrary->queryString('idx,w'));
	}
