...
<tr class="bgcol1 bold col1 ht center">
	...
	<td>읽기권한</td>
	<td>등록일</td>
	<td><button type="button" class="red_btn" onclick="window.location='./board_info_form.php?<?=$funcLibrary->queryString('w')?>w=a'">등록</button></td>
</tr>
<?php
	$tableName = 'gh_board';
	$where = "where 1=1";
	$orderby = "idx desc";
	$listResult = $queryLibrary->getList($where,$bindParam,$tableName,$orderby,1,100);
...
<tr class="list col1 ht center">
	<td><?=$number?></td>
	<td class="td2"><a href="board_info_form.php?<?=$funcLibrary->queryString()?>w=u&idx=<?=$d['idx']?>"><?=$d['b_name']?></a></td>
	...
	<td><?=$regdate?></td>
	<td>
		<button type="button" class="black_icon_btn" onclick="window.location='board_info_form.php?<?=$funcLibrary->queryString()?>w=u&idx=<?=$d['idx']?>'">수정</button>
		<button type="button" class="gray_icon_btn" onclick="if(confirm('정말 삭제하시겠습니까?'))location.href='board_info_ok.php?<?=$funcLibrary->queryString()?>w=d&idx=<?=$d['idx']?>';">삭제</button>
	</td>
</tr>
