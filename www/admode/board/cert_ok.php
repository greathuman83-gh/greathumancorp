...
if($w == 'a'){
	...
	if(!$DB->insertInto($table_name, $inputs)){
		...
		$func_library->alert('등록되었습니다.','./cert_list.php?'.$func_library->queryString('idx,w'));
	}
	...
	if(!$DB->updateSet($table_name,$inputs,$where)){
		...
		$func_library->alert('수정되었습니다.','./cert_form.php?'.$func_library->queryString());
	}
	...
	if(!$DB->delete_db($table_name,$where)){
		...
		$func_library->alert('삭제 되었습니다.','./cert_list.php?'.$func_library->queryString('idx,w'));
	}
}else if($w == 'oe'){//순서 변경
	$inputs = array();
	$inputs['num'] = $num;
	...
	if(!$DB->updateSet($table_name,$inputs,$where)){
		...
		$func_library->alert('수정되었습니다.','./cert_list.php?'.$func_library->queryString('idx,w'));
	}
