...
?>
<!--<button type="button" class="black_btn" onclick="window.location='./techpr_sort.php?<?=$func_library->queryString()?>'" style="margin-bottom:5px;">순서관리</button> -->
...
<tr class="bgcol1 bold col1 ht center">
	...
	<td>타이틀</td>
	<td>등록일</td>
	<td><button type="button" class="red_btn" onclick="window.location='./techpr_form.php?<?=$func_library->queryString('w')?>w=a'">등록</button></td>
</tr>
<?php
	$where = "where 1=1";
	...
	$thumb_img = $d['file1'] ? '<img src="'.$gh_path.'data/techpr/'.$d['file1'].'" width="100" style="vertical-align:middle">' : '';
	$main_icon = $d['t_main'] == '1' ? '<button type="button" class="blue_icon_btn">메인</button> ' : '';
?>
<tr class="list col1 ht center">
	<td><?=$number?></td>
	<!-- <td>
		<form action="./techpr_ok.php?<?=$func_library->queryString()?>w=oe&idx=<?=$d['idx']?>" method="post">
	...
	<td>
		<a href="./techpr_form.php?<?=$func_library->queryString('pg,idx,w')?>w=u&idx=<?=$d['idx']?>"><?= $main_icon ?>><?=$d['title']?></a>
	</td>
	<td><?=$regdate?></td>
	<td>
		<button type="button" class="black_icon_btn" onclick="window.location='./techpr_form.php?<?=$func_library->queryString()?>w=u&idx=<?=$d['idx']?>'">수정</button>
		<button type="button" class="gray_icon_btn" onclick="if(confirm('정말 삭제하시겠습니까?'))location.href='./techpr_ok.php?<?=$func_library->queryString()?>w=d&idx=<?=$d['idx']?>';">삭제</button>
	</td>
</tr>
