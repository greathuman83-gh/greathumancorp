...
<tr>
	<!-- <td align="left"><button type="button" class="black_btn" onclick="window.location='./portfolio_sort.php?<?=$func_library->queryString()?>'">순서관리</button></td> -->
	<td align="right">
...
<tr class="bgcol1 bold col1 ht center">
	...
	<td>타이틀</td>
	<td>등록일</td>
	<td><button type="button" class="red_btn" onclick="window.location='./portfolio_form.php?<?=$func_library->queryString('w')?>w=a'">등록</button></td>
</tr>
<?php
	$bind_param = array();
	$where = " where 1=1 ";
	...
	$category_where = " where c_code = :c_code and category = 'portfolio' and depth = '1' ";
...
<tr class="list col1 ht center">
	<td><?=$number?></td>
	<!-- <td>
		<form action="./portfolio_ok.php?<?=$func_library->queryString()?>w=oe&idx=<?=$d['idx']?>" method="post">
	...
	<td><?=$category_text?></td>
	<td class="td2"><?php if($d['thumb_file']){?><img src="<?=$gh_path?>data/portfolio/<?=$d['thumb_file']?>" width="200"><?php }?></td>
	<td class="td2"><a href="./portfolio_form.php?<?=$func_library->queryString('pg,idx,w')?>w=u&idx=<?=$d['idx']?>"><?=$d['title']?></a></td>
	<td><?=$regdate?></td>
	<td>
		<button type="button" class="black_icon_btn" onclick="window.location='./portfolio_form.php?<?=$func_library->queryString()?>w=u&idx=<?=$d['idx']?>'">수정</button>
		<button type="button" class="gray_icon_btn" onclick="if(confirm('정말 삭제하시겠습니까?'))location.href='./portfolio_ok.php?<?=$func_library->queryString()?>w=d&idx=<?=$d['idx']?>';">삭제</button>
	</td>
</tr>
