...
$price_num = (float) preg_replace('/[^0-9.-]/', '', (string)($d['i_price'] ?? ''));
$price_vat = $price_num > 0 ? (int) round($price_num * 0.1) : 0;
$total_price = $price_num + $price_vat;
?>
<form name="fwrite" method="post" action="./invoice_ok.php?<?= $func_library->queryString() ?>" onsubmit="return fwrite_submit(this);" enctype="multipart/form-data" style="margin:0px;">
	<table align="center" cellpadding="0" cellspacing="0" class="adminMenuTable">
		...
		<tr class="ht">
			<td class="td1">VAT</td>
			<td class="td2">
				₩ <div id="priceVat" class="invoice_calc" style="display:inline-block;"><?= number_format($price_vat) ?></div>
			</td>
		</tr>
		<tr>
			<td colspan="2" class="line2"></td>
		...
		<tr class="ht">
			...
			<td class="td2">
				₩ <div id="totalPrice" class="invoice_calc" style="display:inline-block;"><?= number_format($total_price) ?></div>
			</td>
		</tr>
		<tr>
			<td colspan="2" class="line2"></td>
		...
		<tr class="ht">
			...
			<td class="td2">
				...
				<?php if (!empty($d['file1'])) { ?>
					...
					<br>
					<a href="<?= $gh_path ?>board/download.php?board=N&bbsid=invoice&file_name=<?= rawurlencode($d['file1']) ?>&o_file_name=<?= rawurlencode($d['file1_name']) ?>" target="_blank" rel="noopener"><?= htmlspecialchars((string)($d['file1_name'] ?? $d['file1']), ENT_QUOTES, 'UTF-8') ?></a>
