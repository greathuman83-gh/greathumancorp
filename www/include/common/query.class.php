<?php
class QueryLibrary{
	function __construct(private $DBConnect, private $FunctionLibrary) {
	}
	...
	public function getList($where = '',$bindParam = array(),$tableName = '',$orderby = '',$page = '',$listCount = '',$indexID = 'idx',$debug = false) {
		...
		$listResult['listTotal'] = $listTotal;
	...
	//다중 테이블 리스트 
	public function getUnionList($fullQuery, $bindParam = array(), $orderby = '', $page = 1, $listCount = 10, $indexID = 'idx', $discriminator = 'tableName', $debug = false) {
		//기본값 설정
		if (!$page) $page = 1;
		...
		if ($listTotal < 1) {
			return ['result' => [], 'totalPage' => 0, 'listTotal' => 0, 'number' => 0];
		}
		
		//커버링 인덱스를 적용한 메인 쿼리 생성
	...
	public function getData(int $idx, string $tableName, $idxText = '') {
		if($idxText){
			$idxText = $this->FunctionLibrary->cleanXssTags($idxText);
		}else{
			$idxText = 'idx';
		}
	...
	public function getDataCustom( string $tableName = '', string $sort = '', string $column = '', string $where = '', $bindParam = array(), $queryPrint=false) {
		if(!$sort){
			...
			$sort = $this->FunctionLibrary->cleanXssTags($sort);
		}

		if(!$column){
			$column = '*';
		}else{
			$column = $this->FunctionLibrary->cleanXssTags($column);
		}
	...
	public function encryptGetList($where = '',$bindParam = array(),$tableName = '',$orderby = '',$fieldArray = array(),$page = '',$listCount='',$indexID = 'idx',$debug = false){
		...
		$fieldList = '';
		$comma = '';
		foreach($fieldArray as $key => $fieldName){//암호화 처리 필드
			if($key > 0){
				$comma = ',';
			}
			$fieldList .= $comma."CONVERT(AES_DECRYPT(UNHEX($fieldName), '".$fieldName."_encrypt') USING UTF8) as ".$fieldName."_convert ";
		}


		/* 커버링 인덱스 */
		$sql = "SELECT *,$fieldList FROM ".$this->FunctionLibrary->escapeQuery($tableName)." as a JOIN ( ";
		$sql .= "select ".$this->FunctionLibrary->escapeQuery($indexID)." from ".$this->FunctionLibrary->escapeQuery($tableName)." $where";
		$sql .= " order by ";
		foreach($orderby as $key => $val){
			if($key == 0){
	...
	//단일 데이터 조회
	public function encryptGetData($idx,$tableName,$fieldArray = array()){
		$fieldList = '';
		$comma = '';
		...
		$sql = "SELECT *,$fieldList FROM ".$this->FunctionLibrary->escapeQuery($tableName)." WHERE idx = :idx";
		$stmt = $this->DBConnect->prepare($sql);
		$stmt->bindParam(':idx',$idx);
		$stmt->execute();
		$d = $stmt->fetch(PDO::FETCH_ASSOC);
	...
	public function prevPost($tableName = '', int $postIdx = 1, string $column='idx', string $where ='', $pnParam =array(), string $orderby = 'idx asc',string $standard = 'idx'){
		$sql = "select $column from ".$this->FunctionLibrary->escapeQuery($tableName)." where $standard > :postIdx $where order by $orderby limit 1";
		...
		if($prevRow > 0){
			...
			for($i=0;$i<count($column);$i++){
				$prevData[$column[$i]] = $prev[$column[$i]];
			}
		}else{
			$prevData = '';
		}
	...
	public function nextPost($tableName = '', int $postIdx = 1, string $column='idx', string $where ='', $pnParam =array(), string $orderby = 'idx desc',string $standard = 'idx'){
		...
		if($nextRow > 0){
			...
			$nextData = [];
			for($i=0;$i<count($column);$i++){
				$nextData[$column[$i]] = $next[$column[$i]];
			}
		}else{
			$nextData = '';
		}
	...
	/* 게시판 조회수 카운트 */
	public function boardCountUp(int $idx, string $bbsid){
		$sql = "update gh_board_".$this->FunctionLibrary->escapeQuery($bbsid)." set b_count=b_count+1 where idx=:idx";
		$stmt = $this->DBConnect->prepare($sql);
		$stmt->bindParam(':idx',$idx);
		$stmt->execute();
		//return $sql;
	...
	public function logInsert($paramArray = array()){
		...
		$inputs['l_name'] = $paramArray['adminName'];
