...
(function () {
  ...
  nhn.husky.HuskyCore = jindo.$Class({
    ...
    aCallerStack: null,
    bMobile:
      jindo.$Agent().navigator().mobile || jindo.$Agent().navigator().msafari,
    ...
    $init: function (htOptions) {
      ...
      this._fnWaitForPluginReady = jindo
        .$Fn(this._waitForPluginReady, this)
        .bind();
    ...
    _exec: function (msg, args, oEvent) {
      return (this._exec = this.htOptions.oDebugger
        ? this._execWithDebugger
        : this._execWithoutDebugger).call(this, msg, args, oEvent);
    },
    ...
    registerBrowserEvent: function (obj, sEvent, sMessage, aParams, nDelay) {
      aParams = aParams || [];
      var func = nDelay
        ? jindo.$Fn(this.delayedExec, this).bind(sMessage, aParams, nDelay)
    ...
    _execMsgStep: function (sMsgStep, sMsg, args) {
      return (this._execMsgStep = this.htOptions.oDebugger
        ? this._execMsgStepWithDebugger
        : this._execMsgStepWithoutDebugger).call(this, sMsgStep, sMsg, args);
    },
      ...
      if (_rxMsgAppReady.test(sMsgHandler)) {
        ...
        for (i = 0; i < iNumOfPlugins; i++) {
          if (!aPlugins[i]._husky_bRun) {
            ...
            if (
              typeof aPlugins[i].$LOCAL_BEFORE_FIRST == "function" &&
              this._execHandler(aPlugins[i], "$LOCAL_BEFORE_FIRST", [
                sMsgHandler,
                args,
              ]) === false
            ) {
          ...
          if (typeof aPlugins[i].$LOCAL_BEFORE_ALL == "function") {
            if (
              this._execHandler(aPlugins[i], "$LOCAL_BEFORE_ALL", [
                sMsgHandler,
                args,
              ]) === false
            ) {
    ...
    _execHandler: function (oPlugin, sHandler, args) {
      return (this._execHandler = this.htOptions.oDebugger
        ? this._execHandlerWithDebugger
        : this._execHandlerWithoutDebugger).call(this, oPlugin, sHandler, args);
    },
    ...
    _loadLazyFiles: function (sMsg, aArgs, oEvent, aFilenames, nIdx) {
      ...
      if (_htLoadedFile[sFilename]) {
        ...
        jindo.LazyLoading.load(
          nhn.husky.SE2M_Configuration.LazyLoad.sJsBaseURI + "/" + sFilename,
          jindo
            .$Fn(function (sMsg, aArgs, oEvent, aFilenames, nIdx) {
              // 로딩완료된 파일은 상태를 변경하고
              var sFilename = aFilenames[nIdx];
            ...
            .bind(sMsg, aArgs, oEvent, aFilenames, nIdx),
          "utf-8",
        );
      }
    },
...
nhn.W3CDOMRange = jindo.$Class({
  ...
  cloneContents: function () {
    ...
    if (
      oClonedContainers.oEndContainer &&
  ...
  _constructClonedTree: function (aNodes, oClonedParentNode) {
    ...
    var _recurConstructClonedTree = function (
      aAllNodes,
      iCurIdx,
      oClonedParentNode,
    ) {
      ...
      while (
        ...
        iChildIdx = this._recurConstructClonedTree(
          aAllNodes,
          iChildIdx,
          oCurNodeCloneWithChildren,
        );
      }

      // this may trigger an error message in IE when an erroneous script is inserted
      oClonedParentNode.insertBefore(
        oCurNodeCloneWithChildren,
    ...
    this._recurConstructClonedTree(
      aNodes,
      aNodes.length - 1,
      oClonedParentNode,
    );
  ...
  _compareEndPoint: function (oContainerA, iOffsetA, oContainerB, iOffsetB) {
    return this.oBrowserSelection.compareEndPoints(
      oContainerA,
      iOffsetA,
      oContainerB,
      iOffsetB,
    ...
    var oCommonAncestor = this._getCommonAncestorContainer(
      oContainerA,
      oContainerB,
    );
  ...
  setEndAfter: function (refNode, bNoUpdate) {
    ...
    if (refNode.tagName == "BODY") {
      this.setEnd(
        refNode,
        nhn.DOMFix.childNodes(refNode).length,
        true,
        bNoUpdate,
      );
      return;
  ...
  _updateRangeInfo: function () {
    ...
    this.collapsed =
      this.oBrowserSelection.isCollapsed(this) ||
      (this.startContainer === this.endContainer &&
        this.startOffset === this.endOffset);
    ...
    this.commonAncestorContainer =
      this.oBrowserSelection.getCommonAncestorContainer(this);
  ...
  _isCollapsed: function (
    ...
    if (oStartContainer == oEndContainer && iStartOffset == iEndOffset) {
      ...
      var oActualStartNode = this._getActualStartNode(
        oStartContainer,
        iStartOffset,
      );
      ...
      if (
        ...
        (this._getNextNode(oActualEndNode) == oActualStartNode ||
          (oActualEndNode == oActualStartNode &&
            this._isBlankTextNode(oActualEndNode)))
      ) {
        bCollapsed = true;
      }
    }
  ...
  _getNodesInRange: function () {
    ...
    var oStartNode = this._getActualStartNode(
      this.startContainer,
      this.startOffset,
    );
  ...
  .$Class({
    ...
    $init: function (win) {
      ...
      this.rxLineBreaker = new RegExp(
        "^(" + this.sBlockElement + "|" + this.sBlockContainer + ")$",
      );
    ...
    getStartNode: function () {
      ...
      if (this.startContainer.nodeType == 3) {
        ...
        if (
          this.startOffset >= nhn.DOMFix.childNodes(this.startContainer).length
        ) {
          return this._getNextNode(this.startContainer);
        }
    ...
    getNodeAroundRange: function (bBefore, bStrict) {
      ...
      if (
        ...
        oAfterRange = nhn.DOMFix.childNodes(this.startContainer)[
          this.startOffset
        ];
      }
      ...
      if (this.endOffset === 0) {
        ...
        oBeforeRange = nhn.DOMFix.childNodes(this.endContainer)[
          this.endOffset - 1
        ];
      }
    ...
    placeStringBookmark_Webkit: function () {
      ...
      if (
        elInsertBefore.previousSibling &&
        elInsertBefore.previousSibling.tagName == "TD"
      ) {
      ...
      if (
        ...
        elInsertBefore.nextSibling.tagName == "TD"
      ) {
    ...
    getStringBookmark: function (sBookmarkID, bEndBookmark) {
      if (bEndBookmark) {
        return this._document.getElementById(
          this.HUSKY_BOOMARK_END_ID_PREFIX + sBookmarkID,
        );
      } else {
        return this._document.getElementById(
          this.HUSKY_BOOMARK_START_ID_PREFIX + sBookmarkID,
        );
      }
    },
	...
	*/
    ...
    isRangeinRange: function (oAnoterRange, bIncludePartlySelected) {
      var startToStart = this.compareBoundaryPoints(
        this.W3CDOMRange.START_TO_START,
        oAnoterRange,
      );
      var startToEnd = this.compareBoundaryPoints(
        this.W3CDOMRange.START_TO_END,
        oAnoterRange,
      );
      var endToStart = this.compareBoundaryPoints(
        this.W3CDOMRange.ND_TO_START,
        oAnoterRange,
      );
      var endToEnd = this.compareBoundaryPoints(
        this.W3CDOMRange.END_TO_END,
        oAnoterRange,
      );
    ...
    pasteHTML: function (sHTML) {
      ...
      if (
        oStart.oLineBreaker &&
        oStart.oLineBreaker.nodeName === "P" &&
        clone.rxHasBlock.test(sHTML)
      ) {
        ...
        oTmpDiv.style.cssText = oStart.oLineBreaker.style.cssText.replace(
          this._rxTextAlign,
          "",
        ); // text-align 제외
    ...
    _hasTextDecoration: function (oNode, sValue) {
      ...
      if (
        sValue === "line-through" &&
        (oNode.tagName === "S" || oNode.tagName === "STRIKE")
      ) {
        return true;
      }
    ...
    _setTextDecoration: function (oNode, sValue) {
      if (jindo.$Agent().navigator().firefox) {
        // FF
        oNode.style.textDecoration = oNode.style.textDecoration
          ? oNode.style.textDecoration + " " + sValue
          : sValue;
      } else {
        if (sValue === "underline") {
    ...
    _checkTextDecoration: function (oNode) {
      ...
      while (oChildNode) {
        if (oChildNode.nodeType === 1) {
          ...
          bLineThrough =
            bLineThrough ||
            oChildNode.tagName === "S" ||
            oChildNode.tagName === "STRIKE";
        }
      ...
      while (oParentNode && oParentNode.tagName !== "BODY") {
        ...
        if (
          !bLineThrough &&
          this._hasTextDecoration(oParentNode, "line-through")
        ) {
          bLineThrough = true;
    ...
    styleRange: function (
      ...
      sNewSpanMarker,
      bIncludeLI,
      bCheckTextDecoration,
    ) {
      var aStyleParents = (this.aStyleParents = this._getStyleParentNodes(
        sNewSpanMarker,
        bIncludeLI,
      ));
    ...
    expandStart: function () {
      ...
      var elActualStartNode = this._getActualStartNode(
        this.startContainer,
        this.startOffset,
      );
    ...
    expandEnd: function () {
      ...
      var elActualEndNode = this._getActualEndNode(
        this.endContainer,
        this.endOffset,
      );
    ...
    _getStyleParentNodes: function (sNewSpanMarker, bIncludeLI) {
      ...
      for (var i = 0; i < nInitialLength; i++) {
        ...
        if (
          bIncludeLI &&
          oNode.tagName == "LI" &&
          this._isFullyContained(oNode, arAllBottomNodes)
        ) {
    ...
    _findParentSingleSpan: function (oNode) {
      ...
      for (
        var i = 0, nCnt = 0, sValue, oChild, aChildNodes = oNode.childNodes;
        (oChild = aChildNodes[i]);
        i++
      ) {
...
nhn.BrowserSelectionImpl_FF = function () {
  ...
  this.compareEndPoints = function (
    elContainerA,
    nOffsetA,
    elContainerB,
    nOffsetB,
...
nhn.BrowserSelectionImpl_IE = function () {
  ...
  this.compareEndPoints = function (
    ...
    if (
      elContainerA === this.oLastRange.elStartContainer &&
      nOffsetA === this.oLastRange.nStartOffset
    ) {
      oIERangeA = this.oLastRange.oBrowserRange.duplicate();
      oIERangeA.collapse(true);
    } else {
      if (
        elContainerA === this.oLastRange.elEndContainer &&
        nOffsetA === this.oLastRange.nEndOffset
      ) {
        oIERangeA = this.oLastRange.oBrowserRange.duplicate();
    ...
    if (
      elContainerB === this.oLastRange.elStartContainer &&
      nOffsetB === this.oLastRange.nStartOffset
    ) {
      oIERangeB = this.oLastRange.oBrowserRange.duplicate();
      oIERangeB.collapse(true);
    } else {
      if (
        elContainerB === this.oLastRange.elEndContainer &&
        nOffsetB === this.oLastRange.nEndOffset
      ) {
        oIERangeB = this.oLastRange.oBrowserRange.duplicate();
  ...
  this._W3CRange2IERange = function (oW3CRange) {
    ...
    var oStartIERange = this._getIERangeAt(
      oW3CRange.startContainer,
      oW3CRange.startOffset,
    );
    var oEndIERange = this._getIERangeAt(
      oW3CRange.endContainer,
      oW3CRange.endOffset,
    );
  ...
  this._getSelectableNodeAndOffsetForIE = function (oW3CContainer, iW3COffset) {
    ...
    if (oW3CContainer.nodeType == 3) {
      ...
      iNumOfLeftNodesToCount =
        iW3COffset < aChildNodes.length ? iW3COffset : aChildNodes.length;
    }
    //@ room 4 improvement
...
nhn.husky.CorePlugin = jindo.$Class({
  ...
  $ON_LOAD_FULL_PLUGIN: function (
    ...
    if (this.htLazyLoadRequest_plugins[sFilename].nStatus === 2) {
      ...
      this.oApp.exec("MSG_FULL_PLUGIN_LOADED", [
        sFilename,
        sClassName,
        sMsgName,
        oThisRef,
  ...
  _loadFullPlugin: function (
    ...
    jindo.LazyLoading.load(
      nhn.husky.SE2M_Configuration.LazyLoad.sJsBaseURI + "/" + aFilenames[nIdx],
      jindo
        .$Fn(function (
          aFilenames,
          sClassName,
        ...
        .bind(aFilenames, sClassName, sMsgName, oThisRef, oArguments, nIdx),

      "utf-8",
    );
  },

  $ON_MSG_FULL_PLUGIN_LOADED: function (
    aFilenames,
    sClassName,
    sMsgName,
    oThisRef,
    ...
    for (var i = 0, nLen = oThisRef._huskyFLT.length; i < nLen; i++) {
      ...
      var oRemoveFrom =
        oThisRef.$this && oThisRef[sLoaderHandlerName] ? oThisRef : oPluginRef;
    ...
    for (var x in oPlugin) {
      ...
      if (
        oThisRef.$this &&
        (!oThisRef[x] ||
          (typeof oPlugin[x] === "function" && x != "constructor"))
      ) {
...
nhn.husky.SE2M_Toolbar = jindo.$Class({
  ...
  _assignHTMLElements: function (oAppContainer) {
    ...
    this.aAllUI = jindo.$$(
      "[class*=" + this.sUIClassPrefix + "]",
      this.toolbarArea,
    );
    this.elTextTool = jindo.$$.getSingle(
      "div.husky_seditor_text_tool",
      this.toolbarArea,
    ); // [SMARTEDITORSUS-1124] 텍스트 툴바 버튼의 라운드 처리
    //@ec]
    ...
    if (jindo.$$.getSingle("DIV.se2_icon_tool") != null) {
      this.elFirstToolbarItem = jindo.$$.getSingle(
        "DIV.se2_icon_tool UL.se2_itool1>li>button",
      );
    }
  },

  $LOCAL_BEFORE_FIRST: function (sMsg) {
    var aToolItems = jindo.$$(
      ">ul>li[class*=" + this.sUIClassPrefix + "]>button",
      this.elTextTool,
    );
  ...
  $ON_MSG_APP_READY: function () {
    ...
    if (this._htOptions.aDisabled) {
      this._htOptions._sDisabled =
        "," + this._htOptions.aDisabled.toString() + ","; // 버튼을 활성화할때 비교하기 위한 문자열구성
  ...
  $ON_NAVIGATE_TOOLBAR: function (weEvent) {
    ...
    if (
      weEvent.element == this.elLastToolbarItem &&
      weEvent.key().keyCode == TAB_KEY_CODE
    ) {
    ...
    if (
      weEvent.element == this.elFirstToolbarItem &&
      weEvent.key().keyCode == TAB_KEY_CODE
    ) {
      if (weEvent.key().shift) {
        weEvent.stopDefault();
  ...
  $ON_TOGGLE_TOOLBAR_ACTIVE_LAYER: function (
    elLayer,
    elBtn,
    sOpenCmd,
    aOpenArgs,
    ...
    this.oApp.exec("TOGGLE_ACTIVE_LAYER", [
      elLayer,
      "MSG_TOOLBAR_LAYER_SHOWN",
      [elLayer, elBtn, sOpenCmd, aOpenArgs],
      sCloseCmd,
      aCloseArgs,
    ]);
  },
  ...
  $ON_POSITION_TOOLBAR_LAYER: function (elLayer, htOption) {
    ...
    if (sAlign != "right") {
      ...
      if (nLayerRight > nToolbarRight) {
        welLayer.css(
          "right",
          -1 * (nToolbarRight - nLayerRight - nMargin) + "px",
        );
      }
      ...
      if (nLayerLeft < nToolbarLeft) {
        welLayer.css(
          "right",
          -1 * (nToolbarLeft - nLayerLeft + nMargin) + "px",
        );
      }
    }
  },
  ...
  $ON_EVENT_TOOLBAR_MOUSEDOWN: function (weEvent) {
    ...
    while (elTmp) {
      if (
        elTmp.className &&
        elTmp.className.match(/active/) &&
        (elTmp.childNodes.length > 2 ||
          elTmp.parentNode.className.match(/se2_pair/))
      ) {
        return;
      }
  ...
  _enableUI: function (sUIName) {
    ...
    if (
      this._htOptions._sDisabled &&
      this._htOptions._sDisabled.indexOf("," + sUIName + ",") > -1
    ) {
      return;
    }
    var i, nLen;
  ...
  _getAffectedElements: function (el) {
    ...
    if ((elLi = el).tagName == "BUTTON") {
      ...
      //   <BUTTON>
      if (
        (elLi = elLi.parentNode) &&
        elLi.tagName == "LI" &&
        this.rxUI.test(elLi.className)
      ) {
      ...
      elLi = el;
      if (
        (elLi = elLi.parentNode.parentNode) &&
        elLi.tagName == "LI" &&
        (welLi = jindo.$Element(elLi)).hasClass("se2_pair")
      ) {
...
nhn.husky.SE_EditingAreaManager = jindo.$Class({
  ...
  $BEFORE_MSG_APP_READY: function (msg) {
    ...
    this.oApp.exec("ADD_APP_PROPERTY", [
      "elEditingAreaContainer",
      this.elEditingAreaContainer,
    ]);
    this.oApp.exec("ADD_APP_PROPERTY", [
      "welEditingAreaContainer",
  ...
  $ON_MSG_APP_READY: function () {
    ...
    this.sDefaultEditingMode =
      this.htOptions["sDefaultEditingMode"] || this.sDefaultEditingMode;
    ...
    if (!!this.fOnBeforeUnload) {
      ...
      window.onbeforeunload = jindo
        .$Fn(function () {
          ...
          if (
            this.getRawContents() != this.sCurrentRawContents ||
            this.bIsDirty
          ) {
  ...
  $ON_FOCUS: function (isPopupOpening) {
    ...
    if (
      ...
      // 누락된 [SMARTEDITORSUS-1018] 작업분 반영
  ...
  $ON_MSG_EDITING_AREA_RESIZE_STARTED: function () {
    ...
    var elSelectedUI = jindo.$$.getSingle(
      "ul[class^='se2_itool']>li.active",
      this.toolbarArea,
    ...
    if (
      elSelectedUIParent &&
      (elSelectedUIParent.className == "se2_itool2" ||
        elSelectedUIParent.className == "se2_itool4")
    ) {
    ...
    this.iStartingHeight = parseInt(
      this.elEditingAreaContainer.style.height,
      10,
    );
  },
  ...
  $ON_RESIZE_EDITING_AREA_BY: function (ipWidthChange, ipHeightChange) {
    ...
    if (
      ...
      iWidth = this.elEditingAreaContainer.style.width
        ? parseInt(this.elEditingAreaContainer.style.width, 10) + iWidthChange
        : null;
    }

    if (iHeightChange !== 0) {
      iHeight = this.elEditingAreaContainer.style.height
        ? this.iStartingHeight + iHeightChange
        : null;
    }
  ...
  $ON_KEEP_WITHIN_EDITINGAREA: function (elLayer, nHeight) {
    ...
    if (
      nTop + elLayer.offsetHeight >
      this.oApp.elEditingAreaContainer.offsetHeight
    ) {
    ...
    if (
      nLeft + elLayer.offsetWidth >
      this.oApp.elEditingAreaContainer.offsetWidth
    ) {
      elLayer.style.left =
        this.oApp.elEditingAreaContainer.offsetWidth -
        elLayer.offsetWidth +
  ...
  createCoverDiv: function () {
    ...
    this.elEditingAreaContainer.insertBefore(
    ...
    this._fitElementInEditingArea(this.elResizingBoard);
    this.elResizingBoard.style.width =
      this.elEditingAreaContainer.offsetWidth + "px";
  ...
  _cleanContents: function (sContents) {
    return sContents.replace(
      new RegExp("(<img [^>]*>)" + unescape("%uFEFF") + "", "ig"),
      "$1",
    );
  },
...
nhn.husky.SE_EditingArea_WYSIWYG = jindo.$Class({
  ...
  $init: function (iframe) {
    ...
    if (this.htOptions) {
      ...
      this.sBlankPageURL_EmulateIE7 =
        this.htOptions.sBlankPageURL_EmulateIE7 ||
        this.sBlankPageURL_EmulateIE7;
      this.aAddtionalEmulateIE7 =
        this.htOptions.aAddtionalEmulateIE7 || this.aAddtionalEmulateIE7;
    }
    ...
    if (
      oAgent.ie &&
      jindo.$A(this.aAddtionalEmulateIE7).has(oAgent.nativeVersion)
    ) {
    ...
    if (this.oApp.oNavigator.ie) {
      jindo
        .$Fn(function (weEvent) {
          ...
          if (
            oSelection &&
            oSelection.type.toLowerCase() === "control" &&
      ...
      if (!this.getDocument().createRange) {
        jindo
          .$Fn(this._onIEBeforeDeactivate, this)
  ...
  startAutoResize: function () {
    ...
    if (oBrowser.ie && oBrowser.version < 9) {
      ...
      jindo
        ...
        .css({ overflowX: "visible", overflowY: "hidden" });
    }
  ...
  stopAutoResize: function () {
    ...
    jindo
      .$Element(this.getDocument().body)
      .css({ overflow: "visible", overflowY: "visible" });
  ...
  _getResizeHeight: function () {
    ...
    if (!this.elDummy) {
      ...
      this.elDummy.style.cssText =
        "position:absolute !important; left:-9999px !important; top:-9999px !important; z-index: -9999 !important; overflow: auto !important;";
    ...
    if (oCss.lineHeight.indexOf("px") > -1) {
      oCss.lineHeight =
        parseInt(oCss.lineHeight, 10) / parseInt(oCss.fontSize, 10);
    }
  ...
  _setAutoResize: function () {
    ...
    this.nTopBottomMargin =
      this.nTopBottomMargin ||
      parseInt(welBody.css("marginTop"), 10) +
        parseInt(welBody.css("marginBottom"), 10);
    this.nBodyMinHeight =
      this.nBodyMinHeight ||
    ...
    // [SMARTEDITORSUS-1868] msafari 일 경우도 추가
    if (
      (oBrowser.ie && oBrowser.nativeVersion >= 9) ||
      oBrowser.chrome ||
      this.oApp.oNavigator.msafari
    ) {
      // 내용이 줄어도 scrollHeight가 줄어들지 않음
    ...
    if (oCurrentStyle) {
      ...
      if (nLineHeight && /[^\d\.]/.test(nLineHeight)) {
        ...
        if (/\d/.test(nLineHeight) && /[A-Za-z]/.test(nLineHeight)) {
          // 단위형 : 실제 원하는 최종 결과값인 만큼, px 단위형으로 변환만 거친 뒤 return
          ...
          // TODO : "normal", "inherit", "initial" 세분화
          nLineHeight = 1.2;
        }
      }
  ...
  _setBodyHeight: function () {
    if (this.bStopCheckingBodyHeight) {
      // 멈춰야 하는 경우 true, 계속 체크해야 하면 false
    ...
    if (nBodyHeight === 0) {
      // [SMARTEDITORSUS-144] height 가 0 이고 내용이 없으면 크롬10 에서 캐럿이 보이지 않음
    ...
    nNewBodyHeight =
      nScrollHeight > nContainerOffset
        ? nScrollHeight - nMarginTopBottom
        : nMinBodyHeight;
  ...
  _isHorizontalScrollbarVisible: function () {
    ...
    if (
      oDocument.documentElement.clientWidth <
      oDocument.documentElement.scrollWidth
    ) {
  ...
  _onIEBeforeDeactivate: function (wev) {
    this.oApp.delayedExec(
      "IE_CHECK_EXCEPTION_FOR_SELECTION_PRESERVATION",
      null,
      0,
    );
  ...
  $ON_CHANGE_EDITING_MODE: function (sMode, bNoFocus) {
    if (sMode === this.sMode) {
      ...
      if (htBrowser.ie && htBrowser.nativeVersion > 8) {
        var elFirstChild = jindo.$$.getSingle(
          "DIV.husky_seditor_editing_area_container",
        ).childNodes[0];
        if (
          elFirstChild.tagName == "DIV" &&
          elFirstChild.style.zIndex == 1000
        ) {
        ...
        if (aImgChild.length > 0) {
          ...
          while (elChild) {
            ...
            if (
              elChild.nodeType === 3 &&
              (elChild.nodeValue === "&nbsp;" ||
                elChild.nodeValue === unescape("%u00A0"))
            ) {
  ...
  $ON_FOCUS_N_CURSOR: function (bEndCursor, sId) {
    ...
    if (!oSelection.collapsed) {
      ...
      // select 영역이 없는 상태에서 bEndCursor 이면 body 맨 뒤로 이동시킨다.
      this.oApp.exec("FOCUS");
  ...
  _getVisibleVerticalPosition: function () {
    ...
    nVisibleHeight = oWindow.innerHeight
      ? oWindow.innerHeight
      : oDocument.documentElement.clientHeight || oDocument.body.clientHeight;
  ...
  _isElementVisible: function (htElementPos, htVisiblePos) {
    return (
      htElementPos.nTop >= htVisiblePos.nTop &&
      htElementPos.nBottom <= htVisiblePos.nBottom
    );
  },
  ...
  _getEditorHeight: function () {
    return (
      this.oApp.elEditingAreaContainer.offsetHeight - this.nTopBottomMargin
    );
  },
  ...
  initIframe: function () {
    try {
      ...
      var sCSSBaseURI =
        !!nhn.husky.SE2M_Configuration.SE2M_CSSLoader &&
      ...
      if (!!nhn.husky.SE2M_Configuration.SE_EditingAreaManager.sCSSBaseURI) {
        sCSSBaseURI =
          nhn.husky.SE2M_Configuration.SE_EditingAreaManager.sCSSBaseURI;
      }
...
nhn.husky.SE_EditingArea_HTMLSrc = jindo.$Class({
  ...
  setIR: function (sIR) {
    ...
    if (
      htBrowser.ie &&
      htBrowser.nativeVersion == 11 &&
      document.documentMode == 11
    ) {
      // Edge 모드의 documentMode 값은 11
...
nhn.husky.SE2M_EditingAreaRuler = jindo.$Class({
  ...
  _adjustWysiwygWidth: function () {
    ...
    var sStyle =
      "{" +
      this.htConfig[this.nRulerWidth].sStyle
        .replace(/;/gi, ",")
        .replace(/\"/gi, "") +
      "}";
    var oStyle = jindo.$Json(
      sStyle.replace(/(\w+)\s?:\s?([\w\s]*[^,}])/gi, '$1:"$2"'),
    );
...
nhn.husky.SE_EditingAreaVerticalResizer = jindo.$Class({
  ...
  isUseVerticalResizer: function () {
    return typeof this.htConversionMode === "undefined" ||
      typeof this.htConversionMode.bUseVerticalResizer === "undefined" ||
      this.htConversionMode.bUseVerticalResizer === true
      ? true
      : false;
  },
  ...
  checkResizeGripPosition: function (bExpand) {
    ...
    var nGap =
      jindo.$Element(this.oResizeGrip).offset().top -
      oDocument.scrollPosition().top +
      25 -
      oDocument.clientSize().height;
    ...
    if (bExpand) {
      if (
        this.nEditingAreaMinHeight >
        this.oApp.getEditingAreaHeight() - nGap
      ) {
        // [SMARTEDITORSUS-822] 수정 모드인 경우에 대비
        nGap =
          -1 * (this.nEditingAreaMinHeight - this.oApp.getEditingAreaHeight());
      }
  ...
  _assignHTMLElements: function (elAppContainer, htConversionMode) {
    ...
    this.elModeToolbar = jindo.$$.getSingle(
      "DIV.se2_conversion_mode",
      elAppContainer,
    );
    //@ec]

    this.welNoticeLayer = jindo.$Element(
      jindo.$$.getSingle("DIV.husky_seditor_resize_notice", elAppContainer),
    );
...
nhn.husky.SE_WYSIWYGEnterKey = jindo.$Class({
  ...
  $ON_MSG_APP_READY: function () {
    ...
    this.tmpTextNode = this.oSelection._document.createTextNode(
      unescape("%u00A0"),
    ); // 공백(&nbsp;) 추가 시 사용할 노드
    jindo
      .$Fn(this._onKeyDown, this)
      .attach(this.oApp.getWYSIWYGDocument(), "keydown");
  },
  ...
  _addBlankText: function (oSelection) {
    ...
    for (i = 0, nLen = oNodes.length; i < nLen; i++) {
      ...
      if (
        oNode.id.indexOf(oSelection.HUSKY_BOOMARK_START_ID_PREFIX) > -1 ||
      ...
      if (
        !oNodeChild ||
        (oNodeChild.nodeType == 3 &&
          nhn.husky.SE2M_Utils.isBlankTextNode(oNodeChild)) ||
        (oNodeChild.nodeType == 1 &&
          oNode.childNodes.length == 1 &&
          (oNodeChild.id.indexOf(oSelection.HUSKY_BOOMARK_START_ID_PREFIX) >
            -1 ||
            oNodeChild.id.indexOf(oSelection.HUSKY_BOOMARK_END_ID_PREFIX) > -1))
      ) {
  ...
  _addCursorHolder: function (elWrapper) {
    ...
    if (
      elWrapper.innerHTML == "" ||
      (elStyleOnlyNode = this._getStyleOnlyNode(elWrapper))
    ) {
  ...
  _wrapBlock: function (oEvent, sWrapperTagName) {
    ...
    if (
      !oStart.bParentBreak ||
      oSelection.rxBlockContainer.test(oStart.oLineBreaker.tagName)
    ) {
      oEvent.stop();
      ...
      if (
        !!oStart.oNode.parentNode &&
        oStart.oNode.parentNode.nodeType !== 11
      ) {
        //	LineBreaker 로 감싸서 분리
        oSWrapper = this.oApp
          .getWYSIWYGDocument()
          .createElement(this.sLineBreaker);
        ...
        if (
          oEWrapper.lastChild !== null &&
          oEWrapper.lastChild.tagName == "BR"
        ) {
    ...
    if (this.htBrowser.firefox) {
      if (
        ...
        elBookmark.nextSibling.tagName == "IFRAME"
      ) {
      ...
      bAddUnderline =
        elParentNode.tagName === "U" ||
        nhn.husky.SE2M_Utils.findAncestorByTagName("U", elParentNode) !== null;
      bAddLineThrough =
        elParentNode.tagName === "S" ||
        elParentNode.tagName === "STRIKE" ||
        (nhn.husky.SE2M_Utils.findAncestorByTagName("S", elParentNode) !==
          null &&
          nhn.husky.SE2M_Utils.findAncestorByTagName("STRIKE", elParentNode) !==
            null);
      ...
      if (bAddUnderline || bAddLineThrough) {
        setTimeout(
          jindo
            .$Fn(this._addTextDecorationTag, this)
            .bind(bAddUnderline, bAddLineThrough),
          0,
        );
      ...
      setTimeout(
        jindo.$Fn(this._addExtraCursorHolder, this).bind(elParentNode),
        0,
      );
    } else {
      // [SMARTEDITORSUS-1797] 엔터시 공백문자를 &nbsp; 로 변환
  ...
  _addTextDecorationTag: function (bAddUnderline, bAddLineThrough) {
    ...
    if (
      oTargetNode.tagName === "U" ||
      oTargetNode.tagName === "S" ||
      oTargetNode.tagName === "STRIKE"
    ) {
      return;
    }
  ...
  _getStyleNode: function (elNode) {
    while (
      elNode.firstChild &&
      this.oSelection._isBlankTextNode(elNode.firstChild)
    ) {
    ...
    if (
      elFirstChild.nodeType === 3 ||
      (elFirstChild.nodeType === 1 &&
        (elFirstChild.tagName == "IMG" ||
          elFirstChild.tagName == "BR" ||
          elFirstChild.tagName == "HR" ||
          elFirstChild.tagName == "IFRAME"))
    ) {
      return elNode;
    }
  ...
  _getStyleOnlyNode: function (elNode) {
    ...
    if (elNode.firstChild.nodeType === 3) {
      return nhn.husky.SE2M_Utils.isBlankTextNode(elNode.firstChild)
        ? elNode
        : null;
  ...
  _insertBR: function (oEvent) {
    ...
    if (!this.htBrowser.ie) {
      ...
      if (oEnd.bParentBreak) {
        while (
          oEnd.oNode &&
          oEnd.oNode.nodeType == 3 &&
          oEnd.oNode.nodeValue == ""
        ) {
...
nhn.husky.SE2M_EditingModeChanger = jindo.$Class({
  ...
  _assignHTMLElements: function (elAppContainer) {
    ...
    this.elWYSIWYGButton = jindo.$$.getSingle(
      "BUTTON.se2_to_editor",
      elAppContainer,
    );
    this.elHTMLSrcButton = jindo.$$.getSingle(
      "BUTTON.se2_to_html",
      elAppContainer,
    );
    this.elTEXTButton = jindo.$$.getSingle(
      "BUTTON.se2_to_text",
      elAppContainer,
    );
    this.elModeToolbar = jindo.$$.getSingle(
  ...
  $ON_MSG_APP_READY: function () {
    if (this.oApp.htOptions.bOnlyTextMode) {
      ...
      this.oApp.registerBrowserEvent(
        this.elWYSIWYGButton,
        "click",
        "EVENT_CHANGE_EDITING_MODE_CLICKED",
        ["WYSIWYG"],
      );
      this.oApp.registerBrowserEvent(
        this.elHTMLSrcButton,
        "click",
        "EVENT_CHANGE_EDITING_MODE_CLICKED",
        ["HTMLSrc"],
      );
  ...
  isUseModeChanger: function () {
    return typeof this.htConversionMode === "undefined" ||
      typeof this.htConversionMode.bUseModeChanger === "undefined" ||
      this.htConversionMode.bUseModeChanger === true
      ? true
      : false;
  },
  ...
  $ON_EVENT_CHANGE_EDITING_MODE_CLICKED: function (sMode, bNoAlertMsg) {
    if (sMode == "TEXT") {
      ...
      if (sContent.length > 0 && !bNoAlertMsg) {
        if (
          !confirm(this.oApp.$MSG("SE2M_EditingModeChanger.confirmTextMode"))
        ) {
          return false;
        }
      }
...
nhn.husky.SE_PasteHandler = jindo.$Class({
          ...
          .$Fn(function () {
            ...
            if (this.zwspStart) {
              ...
              if (typeof this.zwspStart.nodeValue == "unknown") {
                if (
                  typeof this.zwspStart.parentNode != "unknown" &&
                ...
                // [SMARTEDITORSUS-1676] 이전 로직
                if (this.zwspStart.nodeValue) {
                  this.zwspStart.nodeValue = this.zwspStart.nodeValue.replace(
                    rxZwspStart,
                    "",
                  );
                }
            ...
            if (this.zwspEnd) {
              ...
              if (typeof this.zwspEnd.nodeValue == "unknown") {
                if (
                  typeof this.zwspEnd.parentNode != "unknown" &&
                ...
                if (this.zwspEnd.nodeValue) {
                  this.zwspEnd.nodeValue = this.zwspEnd.nodeValue.replace(
                    rxZwspEnd,
                    "",
                  );
                }
            ...
            try {
              ...
              if (typeof JEagleEyeClient != "undefined") {
                var el =
                  "http://blog.naver.com/hp_SE_PasteHandler.js/_handlePaste";
  ...
  _securePasteArea: function () {
    ...
    if (elNextToStartBookmark) {
      ...
      elStartBookmark.parentNode.insertBefore(
        this.zwspStart,
        elNextToStartBookmark,
      );
    } else {
  ...
  _filterPastedContents: function (sOriginalContent, bUseTableFilter) {
    ...
    var sMatch, // 판별식과 일치하는 부분
      ...
      rxMultiParagraphIndicator = new RegExp(
        "^(" + aMultiParagraphIndicator.join("|") + ")$",
        "i",
      ),
    ...
    var _widthAttribute = "",
      ...
      // [SMARTEDITORSUS-1671]
      rxWidthAttribute =
        /([^\w-])(width[\s]*=[\s]*)(["']?)([A-Za-z0-9.]+[%]?)([;]?["']?)/i,
      rxHeightAttribute =
        /([^\w-])(height[\s]*=[\s]*)(["']?)([A-Za-z0-9.]+[%]?)([;]?["']?)/i,
    ...
    this._doFilter = jindo
      .$Fn(function (sOriginalContent) {
        ...
        while (sOriginalContent != "") {
          ...
          if (rxOpeningAndClosingTag.test(sOriginalContent)) {
              ...
              if (aClass && aClass.length > 0) {
                for (var i = 0, len = aClass.length; i < len; i++) {
                  ...
                  if (rxClassForStyle.test(this._sStyleFromClipboard)) {
                    sMatchedStyle =
                      this._sStyleFromClipboard.match(rxClassForStyle)[1];
                  }
                  ...
                  if (sMatchedStyle) {
                    ...
                    if (!!rxStyle.test(sMatch)) {
                      sMatch = sMatch.replace(
                        rxStyle,
                        "$1$2" + sMatchedStyle + " $3$4",
                      );
                    } else {
                      // style마저 없다면 새로 만들어 준다.
                      sMatch = sMatch.replace(
                        rxOpeningTag_endPart,
                        ' style="' + sMatchedStyle + '"$2',
                      );
                    }
                  }
                }
            ...
            //
              ...
              // [SMARTEDITORSUS-1671] 다중 테이블의 col 캐싱
              if (
                !!aaColumnWidth[nTableDepth] &&
                typeof aaColumnWidth[nTableDepth].length === "number"
              ) {
             ...
             * */
              ...
              sMatch = sMatch.replace(rxBorderWidth, function () {
                return arguments[0]
                  .replace(rxBorderWidth_pointFive_veryFirst, "$11px")
                  .replace(rxBorderWidth_pointFive, " 1px");
              });

              ((nColumnWidth = undefined),
                (aColumnWidth = undefined),
                (_nSpan = undefined));
              ...
              if (!this.htBrowser.ie) {
                  ...
                  if (!rxWidthStyle.test(sMatch) && nColumnWidth) {
                    if (_nSpan) {
                      if (nColumnWidth.indexOf("pt") != -1) {
                        nColumnWidth =
                          parseFloat(nColumnWidth, 10) * _nSpan + "pt";
                      } else if (nColumnWidth.indexOf("px") != -1) {
                        nColumnWidth =
                          parseFloat(nColumnWidth, 10) * _nSpan + "px";
                      }
                    }
                    if (!!rxStyle.test(sMatch)) {
                      sMatch = sMatch.replace(
                        rxStyle,
                        "$1$2width:" + nColumnWidth + "; $3$4",
                      );
                    } else {
                      // style마저 없다면 새로 만들어 준다.
                      sMatch = sMatch.replace(
                        rxOpeningTag_endPart,
                        ' style="width:' + nColumnWidth + ';"$2',
                      );
                    }
                  }
                  // Previous below
                ...
                if (!rxHeightStyle.test(sMatch) && !!nRowHeight) {
                  ...
                  if (!!rxStyle.test(sMatch)) {
                    sMatch = sMatch.replace(
                      rxStyle,
                      "$1$2height:" + nRowHeight + "; $3$4",
                    );
                  } else {
                    // style마저 없다면 새로 만들어 준다.
                    sMatch = sMatch.replace(
                      rxOpeningTag_endPart,
                      ' style="height:' + nRowHeight + ';"$2',
                    );
                  }
                  // --[SMARTEDITORSUS-1671]
                }
            ...
            // 공백문자는 일단 그대로 통과시킴. 차후 처리 방안이 있을지 논의 필요
          ...
          // 지금까지의 조건에 부합하지 않는 모든 태그는 예외 태그로 처리한다. MS Word의 <o:p> 등이 해당.
            ...
            // <*unknown*> : similar to rxOpeningCommentTag case
        ...
        if (!this.htBrowser.ie && nParagraphChangeCount == 0) {
          // --[SMARTEDITORSUS-1862]
          var rxParagraphContainer = new RegExp(
    ...
    var sFilteredContents = bUseTableFilter
      ? this._filterTableContents(sOriginalContent)
      : this._doFilter(sOriginalContent);
  ...
  _filterTableContents: function (sOriginalContents) {
    ...
    if (this.htBrowser.ie) {
      ...
      if (_nStartIndexLength > 0 && _nStartIndexLength == _nEndIndexLength) {
        // <table/> 쌍이 정상적으로 편성되었다면, 열린 횟수와 닫힌 횟수가 같아야 함
        /**
  ...
  _loadToPasteHelper: function () {
    ...
    if (
      ...
      for (var i = 0, len = _aGoesPreviousParagraph.length; i < len; i++) {
        this.elPasteHelper.appendChild(
          _aGoesPreviousParagraph[i].cloneNode(true),
        );
      }
  ...
  _loadToBody: function () {
    ...
    try {
      ...
      var aConversionIndex_original = this._markMatchedElementIndex(
        aSelectedNode_original,
       ...
       * */
      var oSelection_parent = this.oApp.getSelection(
        this.oApp.getWYSIWYGWindow().parent,
      );
      ...
      var aConversionIndex_filtered = this._markMatchedElementIndex(
        aSelectedNode_filtered,
        this.aConversionTarget,
      );
      var nDiff_original_filtered =
        aConversionIndex_original.length - aConversionIndex_filtered.length;

      // As-Is 컨텐츠를 To-Be 컨텐츠로 교체
      if (
        aConversionIndex_original.length > 0 &&
        aConversionIndex_original.length == aConversionIndex_filtered.length
      ) {
        var nConversionIndex_original,
          nConversionIndex_filtered,
          elConversion_as_is,
          elConversion_to_be;

        for (var i = 0, len = aConversionIndex_filtered.length; i < len; i++) {
          nConversionIndex_original =
            aConversionIndex_original[i + nDiff_original_filtered];
          nConversionIndex_filtered = aConversionIndex_filtered[i];

          elConversion_as_is =
            aSelectedNode_original[nConversionIndex_original];
          elConversion_to_be =
            aSelectedNode_filtered[nConversionIndex_filtered];
          if (!/__se_tbl/.test(elConversion_as_is.className)) {
            elConversion_as_is.parentNode.replaceChild(
              elConversion_to_be.cloneNode(true),
              elConversion_as_is,
            );
          }
        }
      ...
      // oSelectionClone 복원
      var elEndBookmark = oSelection.getStringBookmark(this._sBM, true);
      elEndBookmark.parentNode.insertBefore(
        this.oSelectionClone.cloneNode(true),
        elEndBookmark,
      );
   ...
   * --[SMARTEDITORSUS-1676]
   * */
  _markMatchedElementIndex: function (
    aNodeList,
    aTagName,
    aFilter,
    sFilterLogic,
...
nhn.husky.SE2M_ExecCommand = jindo.$Class({
  ...
  _rxCmdInline:
    /^(?:bold|underline|italic|strikethrough|superscript|subscript)$/i, // inline element 가 생성되는 command
  ...
  $ON_MSG_APP_READY: function () {
    ...
    this.oApp.exec("REGISTER_UI_EVENT", [
      ...
      "click",
      "EXECCOMMAND",
      ["justifyleft", false, false],
    ]);
    this.oApp.exec("REGISTER_UI_EVENT", [
      "justifycenter",
      "click",
      "EXECCOMMAND",
      ["justifycenter", false, false],
    ]);
    this.oApp.exec("REGISTER_UI_EVENT", [
      "justifyright",
      "click",
      "EXECCOMMAND",
      ["justifyright", false, false],
    ]);
    this.oApp.exec("REGISTER_UI_EVENT", [
      "justifyfull",
      "click",
      "EXECCOMMAND",
      ["justifyfull", false, false],
    ]);
    this.oApp.exec("REGISTER_UI_EVENT", [
      "orderedlist",
      "click",
      "EXECCOMMAND",
      ["insertorderedlist", false, false],
    ]);
    this.oApp.exec("REGISTER_UI_EVENT", [
      "unorderedlist",
      "click",
  ...
  _isTextBetweenTable: function (oNode) {
    ...
    if (oNode && oNode.nodeType === 3) {
      ...
      if (
        ...
        this._rxTable.test(oTmpNode.nodeName)
      ) {
        return true;
      }
      if (
  ...
  $BEFORE_EXECCOMMAND: function (sCommand, bUserInterface, vValue, htOptions) {
    ...
    for (
      var i = 0, aNodes = oSelection.getNodes(), oNode;
      (oNode = aNodes[i]);
      i++
    ) {
  ...
  _checkBlockQuoteCondition_IE: function () {
    ...
    if (
      (htBrowser.ie &&
        htBrowser.nativeVersion >= 9 &&
        htBrowser.nativeVersion <= 11 &&
        htBrowser.version >= 9 &&
        htBrowser.version <= 11) ||
      (this.oApp.oAgent.os().winxp &&
        htBrowser.ie &&
        htBrowser.nativeVersion <= 8)
    ) {
      var oSelection = this.oApp.getSelection();
      var elCommonAncestorContainer = oSelection.commonAncestorContainer;
      var htAncestor_blockquote =
        nhn.husky.SE2M_Utils.findAncestorByTagNameWithCount(
          "BLOCKQUOTE",
      ...
      if (elBlockquote) {
        var htAncestor_cell =
          nhn.husky.SE2M_Utils.findClosestAncestorAmongTagNamesWithCount(
            ["td", "th"],
            elCommonAncestorContainer,
          );
        if (htAncestor_cell.elNode) {
          if (
            htAncestor_cell.nRecursiveCount >
            htAncestor_blockquote.nRecursiveCount
          ) {
  ...
  $ON_EXECCOMMAND: function (sCommand, bUserInterface, vValue) {
    ...
    bUserInterface =
      bUserInterface == "" || bUserInterface ? bUserInterface : false;
    ...
    if (bSelectedBlock) {
      if (sCommand == "indent") {
        ...
        this.oApp.exec("SET_LINE_BLOCK_STYLE", [
          null,
          jindo.$Fn(this._outdentMargin, this).bind(),
        ]);
      } else {
      ...
      switch (sCommand) {
        ...
        case "outdent":
          ...
          if (sCommand === "indent") {
            this.oApp.exec("SET_LINE_STYLE", [
              null,
              jindo.$Fn(this._indentMargin, this).bind(),
              { bDoNotSelect: true, bDontAddUndoHistory: true },
            ]);
          } else {
            this.oApp.exec("SET_LINE_STYLE", [
              null,
        ...
        default:
          ...
          if (oSelection.collapsed && this._rxCmdInline.test(sCommand)) {
            ...
            if (
              oSingleNode &&
              oSelection._hasCursorHolderOnly(oSingleNode.nextSibling)
            ) {
  ...
  $AFTER_EXECCOMMAND: function (sCommand, bUserInterface, vValue, htOptions) {
    ...
    if (/^insertorderedlist|insertunorderedlist$/i.test(sCommand)) {
      ...
      // --[SMARTEDITORSUS-985][SMARTEDITORSUS-1740]
      this._fixCorruptedBlockQuote(
        sCommand === "insertorderedlist" ? "OL" : "UL",
      ); // IE
    ...
    if (/^justify*/i.test(sCommand)) {
      this._fixAlign(
        sCommand === "justifyfull" ? "justify" : sCommand.substring(7),
      );
    }
  ...
  _getAlignNode: function (elNode) {
    if (
      elNode.tagName &&
      (elNode.tagName === "P" || elNode.tagName === "DIV")
    ) {
      return elNode;
    }
  ...
  _fixAlign: function (sAlign) {
    ...
    var removeTableAlign = !this.oNavigator.ie
      ? function () {}
      : function (elNode) {
    ...
    for (var i = 0, nLen = aNodes.length; i < nLen; i++) {
      ...
      if (
        elParentNode &&
        (elNode === elParentNode ||
          jindo.$Element(elNode).isChildOf(elParentNode))
      ) {
        continue;
      }
  ...
  _setBlockExecCommand: function (sCommand, bUserInterface, vValue) {
    ...
    for (var j = 0; j < aNodes.length; j++) {
      ...
      for (var k = 0; k < aChildrenNode.length; k++) {
        if (
          aChildrenNode[k].tagName == "UL" ||
          aChildrenNode[k].tagName == "OL"
        ) {
  ...
  _indentMargin: function (elDiv) {
    ...
    if (elDiv.tagName === "LI") {
      ...
      if (
        ...
        elDiv.previousSibling.tagName &&
        elDiv.previousSibling.tagName === elDiv.parentNode.tagName
      ) {
        ...
        if (
          elDiv.nextSibling &&
          elDiv.nextSibling.tagName &&
          elDiv.nextSibling.tagName === elDiv.parentNode.tagName
        ) {
          aAppend = [elDiv];

          for (
            i = 0, nLen = elDiv.nextSibling.childNodes.length;
            i < nLen;
            i++
          ) {
  ...
  _outdentMargin: function (elDiv) {
    ...
    if (elDiv.tagName === "LI") {
      ...
      if (
        elDiv.previousSibling &&
        elDiv.previousSibling.tagName &&
        elDiv.previousSibling.tagName.match(/LI|UL|OL/)
      ) {
        // 위아래로 sibling li/ol/ul가 있다면 ol/ul를 2개로 나누어야됨
        if (
          elDiv.nextSibling &&
          elDiv.nextSibling.tagName &&
          elDiv.nextSibling.tagName.match(/LI|UL|OL/)
        ) {
          ...
          elParentNode.parentNode.insertBefore(
            elNewParent,
            elParentNode.nextSibling,
          );
  ...
  _fixCorruptedBlockQuote: function (sTagName) {
    ...
    el.innerHTML = elCorruptedBlockQuote.outerHTML.replace(
      "<" + sTagName,
      "<BLOCKQUOTE",
    );
    elCorruptedBlockQuote.parentNode.insertBefore(
      el.firstChild,
      elCorruptedBlockQuote,
    );
  ...
  _removeBlockQuote: function () {
    ...
    while (elNode) {
      if (elNode.tagName === "LI") {
        elLi = elNode;
        elNode =
          elNode.style.cssText === "display: inline !important; "
            ? elNode.parentNode
            : null;
      } else if (
        elNode.tagName === "SPAN" &&
        elNode.className === sVendorSpanClass
      ) {
        elVendorSpan = elNode;
  ...
  _extendBlock: function () {
    ...
    if (
      !(
        oStartContainer === oEndContainer &&
        oStartContainer.nodeType === 1 &&
        oStartContainer.tagName === "P"
      )
    ) {
      return;
    }
    ...
    if (oSelection.collapsed && !bSelectedBlock) {
      ...
      if (bNewSpan) {
        if (
          ...
          oSelection.startOffset === 0
        ) {
          var oVeryFirstNode = oSelection._getVeryFirstRealChild(
          ...
          if (bAppendable && elTmp.nodeType == 1 && elTmp.tagName == "BR") {
            // [SMARTEDITORSUS-311] [FF4] Cursor Holder 인 BR 의 하위노드로 SPAN 을 추가하여 발생하는 문제
            oSelection.selectNode(oVeryFirstNode);
            oSelection.collapseToStart();
            oSelection.insertNode(oSpan);
          } else if (
            bAppendable &&
            oVeryFirstNode.tagName != "IFRAME" &&
            oVeryFirstNode.appendChild &&
    ...
    if (bSelectedBlock) {
      ...
      if (jindo.$Agent().navigator().firefox) {
        ...
        for (var i = 0, nLen = aStyleParents.length; i < nLen; i++) {
          ...
          if (
            elNode.nextSibling &&
            elNode.nextSibling.tagName == "BR" &&
            !elNode.nextSibling.nextSibling
          ) {
...
nhn.husky.SE_WYSIWYGStyleGetter = jindo.$Class({
  ...
  oStyleMap: {
    ...
    lineHeight: {
      ...
      converter: function (sValue, oStyle) {
        ...
        return (
          Math.ceil(
            (parseInt(sValue, 10) / parseInt(oStyle.fontSize, 10)) * 10,
          ) / 10
        );
      },
    },
  ...
  $ON_EVENT_EDITING_AREA_KEYDOWN: function (oEvent) {
    ...
    // ctrl+a
    if (
      (this.oApp.oNavigator.ie || this.oApp.oNavigator.firefox) &&
      oKeyInfo.ctrl &&
      oKeyInfo.keyCode == 65
    ) {
      this.oApp.exec("RESET_STYLE_STATUS");
  ...
  $ON_RESET_STYLE_STATUS: function () {
    ...
    for (var sAttributeName in this.oStyle) {
      ...
      this.oApp.exec("MSG_STYLE_CHANGED", [
        sAttributeName,
        this.oStyle[sAttributeName],
      ]);
    }
  },
  ...
  _getStyle: function () {
    var oStyle;
    if (nhn.CurrentSelection.isCollapsed()) {
      oStyle = this._getStyleOf(
        nhn.CurrentSelection.getCommonAncestorContainer(),
      );
    } else {
      var oSelection = this.oApp.getSelection();

      var funcFilter = function (oNode) {
        if (!oNode.childNodes || oNode.childNodes.length == 0) return true;
        else return false;
      };
    ...
    for (attributeName in oStyle) {
      if (this.oStyleMap[attributeName].converter) {
        oStyle[attributeName] = this.oStyleMap[attributeName].converter(
      ...
      if (this.oStyle[attributeName] != oStyle[attributeName]) {
        ...
        if (
          ...
          if (
            ...
            this.oStyle["fontFamily"] != oStyle["fontFamily"]
          ) {
            continue;
          }
        }
      ...
      if (attribute.type && attribute.type == "Value") {
        try {
          if (attribute.css) {
            ...
            oStyle[styleName] = this.oDocument.queryCommandState(
              attribute.command,
            );
          } else {
            // todo
...
nhn.husky.SE2M_FontSizeWithLayerUI = jindo.$Class({
  ...
  _assignHTMLElements: function (elAppContainer) {
    ...
    this.aLIFontSizes = jindo
      .$A(jindo.$$("LI", this.oDropdownLayer))
  ...
  $ON_MSG_APP_READY: function () {
    ...
    for (var i = 0; i < this.aLIFontSizes.length; i++) {
      var sFsInit = this._getFontSizeFromLI(this.aLIFontSizes[i]);
      if (sFsInit) {
        this.oApp.registerBrowserEvent(
          this.aLIFontSizes[i],
          "click",
  ...
  $ON_MSG_STYLE_CHANGED: function (sAttributeName, sAttributeValue) {
    if (sAttributeName == "fontSize") {
      // px 단위 유지(툴바 표시 및 목록과 일치). 과거 pt 스타일은 px로 환산해 표시.
      if (this._rxPX.test(sAttributeValue)) {
        var numPx = parseFloat(sAttributeValue.replace(this._rxPX, ""));
        if (numPx > 0) {
          sAttributeValue = Math.round(numPx) + "px";
        } else {
          sAttributeValue = this.sDefaultText;
        }
      } else if (this._rxPT.test(sAttributeValue)) {
        var numPt = parseFloat(sAttributeValue.replace(this._rxPT, ""));
        var integerPart = Math.floor(numPt);
        var decimalPart = numPt - integerPart;
        if (decimalPart >= 0 && decimalPart < 0.25) {
          numPt = integerPart + 0;
        } else if (decimalPart >= 0.25 && decimalPart < 0.75) {
          numPt = integerPart + 0.5;
        } else {
          numPt = integerPart + 1;
        }
        sAttributeValue = Math.round((numPt * 96) / 72) + "px";
      }
  ...
  _getMatchingLI: function (sFontSize) {
    ...
    for (var i = 0; i < this.aLIFontSizes.length; i++) {
      ...
      sLiFs = this._getFontSizeFromLI(elLi);
      if (sLiFs && sLiFs.toLowerCase() == sFontSize) {
        return elLi;
      }
    }
  ...
  _getFontSizeFromLI: function (elLi) {
    ...
    n = btn.firstChild;
    while (n) {
      if (n.nodeType === 1 && n.tagName === "SPAN" && n.style) {
        fs = n.style.fontSize;
        if (fs) {
          return fs;
        }
...
nhn.husky.SE2M_LineStyler = jindo.$Class({
      ...
      jindo
        .$Fn(function (firstNode, lastNode) {
          ...
          if (aNodes.length > 0) {
            ...
            var elFirstTD = nhn.husky.SE2M_Utils.findAncestorByTagName(
              "TD",
              firstNode,
            );
            var elLastTD = nhn.husky.SE2M_Utils.findAncestorByTagName(
              "TD",
              lastNode,
            );
            firstNode =
              elFirstTD || !firstNode ? aNodes[0].firstChild : firstNode;
            lastNode =
              elLastTD || !lastNode
                ? aNodes[aNodes.length - 1].lastChild
                : lastNode;
          }
  ...
  setLineStyle: function (sStyleName, styleValue, htOptions, nodes) {
    ...
    if (bWrapperCreated && (!htOptions || !htOptions.bDoNotSelect)) {
      setTimeout(
        jindo
          .$Fn(function (startNode, endNode, htOptions) {
            if (startNode == endNode) {
              this.oSelection.selectNodeContents(startNode);

              if (
                startNode.childNodes.length == 1 &&
                startNode.firstChild.tagName == "BR"
              ) {
          ...
          .bind(startNode, endNode, htOptions),
        0,
      );
    }
  },
  ...
  setLineBlockStyle: function (sStyleName, styleValue, htOptions) {
    ...
    for (var j = 0; j < aNodes.length; j++) {
      ...
      for (var k = 0, m = 0; k < aTempNodes.length; k++) {
        if (
          aTempNodes[k].nodeType == 3 ||
          (aTempNodes[k].tagName == "BR" && k == 0)
        ) {
    ...
    var txtFilter = function (oNode) {
      ...
      if (
        (oNode.nodeType == 3 &&
          ...
          oNode.nodeValue != "" &&
          oNode.nodeValue != "\uFEFF") ||
        (oNode.tagName == "LI" && oNode.innerHTML == "") ||
  ...
  _getSelectedNodes: function (bDontUpdate) {
    ...
    if (
      this.oSelection.endContainer.tagName == "LI" &&
...
nhn.husky.SE2M_ColorPalette = jindo
  .$Class({
    ...
    URL_COLOR_UPDATE: "",
    sRecentColorTemp:
      '<li><button type="button" title="{RGB_CODE}" style="background:{RGB_CODE}"><span><span>{RGB_CODE}</span></span></button></li>',
    ...
    _assignHTMLElements: function (oAppContainer) {
      ...
      if (this.bUseRecentColor) {
        this.elColorPaletteLayerRecent = jindo.$$.getSingle(
    ...
    $ON_EVENT_MOUSEOVER_COLOR_PALETTE: function (oEvent) {
      ...
      while (
        elHovered &&
        elHovered.tagName &&
        elHovered.tagName.toLowerCase() != "li"
      ) {
      ...
      if (
        elHovered.className == "" ||
        !elHovered.className ||
        typeof elHovered.className == "undefined"
      ) {
    ...
    $ON_APPLY_COLOR: function (elButton) {
      ...
      // 색상 버튼인 경우
      welColorParent = jindo.$Element(
        elButton.parentNode.parentNode.parentNode,
      );
    ...
    _redrawRecentColorElement: function () {
      ...
      for (i = 0; i < nRecentColor; i++) {
        aRecentColorHtml.push(
          this.sRecentColorTemp.replace(/\{RGB_CODE\}/gi, this.aRecentColor[i]),
        );
      }
...
nhn.husky.SE2M_FontColor = jindo.$Class({
  ...
  _assignHTMLElements: function (elAppContainer) {
    ...
    this.elLastUsed = jindo.$$.getSingle(
      "SPAN.husky_se2m_fontColor_lastUsed",
      elAppContainer,
    );

    this.elDropdownLayer = jindo.$$.getSingle(
      "DIV.husky_se2m_fontcolor_layer",
      elAppContainer,
    );
  ...
  $ON_MSG_APP_READY: function () {
    ...
    this.oApp.registerLazyMessage(
      ["APPLY_LAST_USED_FONTCOLOR", "TOGGLE_FONTCOLOR_LAYER"],
...
nhn.husky.SE2M_BGColor = jindo.$Class({
  ...
  _assignHTMLElements: function (elAppContainer) {
    //@ec[
    this.elLastUsed = jindo.$$.getSingle(
      "SPAN.husky_se2m_BGColor_lastUsed",
      elAppContainer,
    );
    ...
    this.elBGColorList = jindo.$$.getSingle(
      "UL.husky_se2m_bgcolor_list",
      elAppContainer,
    );
    this.elPaletteHolder = jindo.$$.getSingle(
  ...
  $ON_MSG_APP_READY: function () {
    ...
    this.oApp.registerLazyMessage(
      ["APPLY_LAST_USED_BGCOLOR", "TOGGLE_BGCOLOR_LAYER"],
      ["hp_SE2M_BGColor$Lazy.js"],
    );
  },
...
nhn.husky.SE2M_Hyperlink = jindo.$Class({
  ...
  _assignHTMLElements: function (elAppContainer) {
    ...
    this.oHyperlinkLayer = jindo.$$.getSingle(
      "div.se2_layer",
      this.oHyperlinkButton,
    );
    this.oLinkInput = jindo.$$.getSingle(
      "INPUT[type=text]",
      this.oHyperlinkLayer,
    );

    this.oBtnConfirm = jindo.$$.getSingle(
      "button.se2_apply",
      this.oHyperlinkLayer,
    );
    this.oBtnCancel = jindo.$$.getSingle(
      "button.se2_cancel",
      this.oHyperlinkLayer,
    );
  ...
  $ON_MSG_APP_READY: function () {
    ...
    this.oApp.registerLazyMessage(
      ["TOGGLE_HYPERLINK_LAYER", "APPLY_HYPERLINK"],
      ["hp_SE2M_Hyperlink$Lazy.js"],
    );
  },
  ...
  $LOCAL_BEFORE_FIRST: function (sMsg) {
    ...
    this.sRXATagMarker = this.sATagMarker
      .replace(/\//g, "\\/")
      .replace(/\./g, "\\.");
    this.oApp.registerBrowserEvent(
      this.oBtnConfirm,
  ...
  irToDb: function (oTmpNode) {
    if (this._bDisabled) {
      // [MUG-1265] 버튼이 사용불가 상태이면 자동변환하지 않는다.
      return;
    }
    //저장 시점에 자동 링크를 위한 함수.
    ...
    var waAllNodes = jindo.$A(
      oTmpRange._getNodesBetween(elFirstNode, elLastNode),
    );
    var aAllTextNodes = waAllNodes
      .filter(function (elNode) {
    ...
    for (var i = 0, nLen = aAllTextNodes.length; i < nLen; i++) {
      ...
      try {
        ...
        elTmpDiv.innerHTML = (sTmpStr + elTmpDiv.innerHTML).replace(
          /(&nbsp|\s)?(((?!http[s]?:\/\/)www\.(?:(?!\&nbsp;|\s|"|').)+)|(http[s]?:\/\/(?:(?!&nbsp;|\s|"|').)+))/gi,
...
nhn.husky.SE2M_FontNameWithLayerUI = jindo.$Class({
  ...
  addAllFonts: function () {
    ...
    if (this.htOptions) {
      ...
      for (i = 0; i < aDefaultFontList.length; i++) {
        this.addFont(
          aDefaultFontList[i][0],
          aDefaultFontList[i][1],
          0,
          "",
          "",
          1,
    ...
    if (!!this.aAdditionalFontList && this.aAdditionalFontList.length > 0) {
      for (i = 0, nLen = this.aAdditionalFontList.length; i < nLen; i++) {
        this.addFont(
          this.aAdditionalFontList[i][0],
          this.aAdditionalFontList[i][1],
          0,
          "",
          "",
          1,
  ...
  _assignHTMLElements: function (elAppContainer) {
    //@ec[
    this.oDropdownLayer = jindo.$$.getSingle(
      "DIV.husky_se_fontName_layer",
      elAppContainer,
    );

    this.elFontNameLabel = jindo.$$.getSingle(
      "SPAN.husky_se2m_current_fontName",
      elAppContainer,
    );
  ...
  _initFontName: function () {
    ...
    this.oApp.registerBrowserEvent(
      this.oDropdownLayer,
      "mouseover",
      "EVENT_FONTNAME_LAYER_MOUSEOVER",
      [],
    );
    this.oApp.registerBrowserEvent(
      this.oDropdownLayer,
  ...
  _addFontInMarkup: function () {
    for (
      var i = 0, elLi, sFontFamily, elSeparator, bUseSeparator;
      (elLi = this.aelFontInMarkup[i]);
      i++
    ) {
      if (elLi.firstChild) {
        sFontFamily = this._getFontFamilyFromLI(elLi)
          .replace(this._rxQuote, "")
  ...
  _attachIEEvent: function () {
    ...
    if (this.htBrowser.nativeVersion < 9) {
      // [SMARTEDITORSUS-187] [< IE9] 최초 paste 시점에 웹폰트 파일을 로드
      this._wfOnPasteWYSIWYGBody = jindo.$Fn(this._onPasteWYSIWYGBody, this);
      this._wfOnPasteWYSIWYGBody.attach(
        this.oApp.getWYSIWYGDocument().body,
        "paste",
      );
    ...
    if (document.documentMode < 9) {
      // [SMARTEDITORSUS-169] [>= IE9] 최초 포커스 시점에 웹폰트 로드
      this._wfOnFocusWYSIWYGBody = jindo.$Fn(this._onFocusWYSIWYGBody, this);
      this._wfOnFocusWYSIWYGBody.attach(
        this.oApp.getWYSIWYGDocument().body,
        "focus",
      );
    ...
    this.welEditingAreaCover = jindo.$Element(
      '<DIV style="width:100%; height:100%; position:absolute; top:0px; left:0px; z-index:1000;"></DIV>',
    );

    this.oApp.welEditingAreaContainer.prepend(this.welEditingAreaCover);
    jindo
      .$Fn(this._onMouseupCover, this)
      .attach(this.welEditingAreaCover.$value(), "mouseup");
  },

  _onFocusWYSIWYGBody: function (e) {
    this._wfOnFocusWYSIWYGBody.detach(
      this.oApp.getWYSIWYGDocument().body,
      "focus",
    );
  ...
  _onPasteWYSIWYGBody: function (e) {
    this._wfOnPasteWYSIWYGBody.detach(
      this.oApp.getWYSIWYGDocument().body,
      "paste",
    );
    this._loadAllBaseFont();
  ...
  _onMouseupCover: function (e) {
    ...
    if (
      ...
      if (this.oApp.getWYSIWYGDocument().body.innerHTML == "<p></p>") {
        this.oApp.getWYSIWYGDocument().body.innerHTML =
          '<p><span id="husky_bookmark_start_INIT"></span><span id="husky_bookmark_end_INIT"></span></p>';
        var oSelection = this.oApp.getSelection();
  ...
  _addFontToMenu: function (sDisplayName, sFontFamily, sSampleText) {
    ...
    elItem.innerHTML = this.elFontItemTemplate.innerHTML
      .replace("@DisplayName@", sDisplayName)
      .replace("FontFamily", sFontFamily)
  ...
  $ON_MSG_STYLE_CHANGED: function (sAttributeName, sAttributeValue) {
    if (sAttributeName == "fontFamily") {
      ...
      if (elLi) {
        ...
        var sPrimaryDisp = this._extractPrimaryNonGenericFont(
          sAttributeValue.toLowerCase(),
        );
        this.elFontNameLabel.innerHTML = sPrimaryDisp || this.sDefaultText;
      }
    }
  },
  ...
  /**
   * font-family 스택에서 제네릭이 아닌 첫 글꼴명만 사용한다.
   * (스택 어디에나 있는 'noto sans kr' 등에 매칭되어 선택 글꼴 표시가 고정되는 것을 방지)
   */
  _extractPrimaryNonGenericFont: function (sFontNameLc) {
    if (!sFontNameLc) {
      return "";
    }
    var aGeneric = {
    ...
    var parts = sFontNameLc.split(",");
    for (var p = 0; p < parts.length; p++) {
      var token = jindo.$S(parts[p].replace(/['"]/gi, "")).trim().$value();
      if (token && !aGeneric[token]) {
        return token;
      }
    }
    return "";
  ...
  _getMatchingLI: function (sFontName) {
    ...
    var sPrimary = this._extractPrimaryNonGenericFont(
      sFontName.replace(/["']/g, "").toLowerCase(),
    );
    if (!sPrimary) {
      return null;
    }
    var elLi, aFontFamily, token, h;
    var aGeneric = {
      serif: 1,
      "sans-serif": 1,
      monospace: 1,
    ...
    for (var i = 0; i < this.aLIFontNames.length; i++) {
      ...
      if (elLi.style && elLi.style.display === "none") {
        continue;
      }
      var sFam = this._getFontFamilyFromLI(elLi);
      if (!sFam) {
        continue;
      }
      aFontFamily = sFam.toLowerCase().split(",");
      for (h = 0; h < aFontFamily.length; h++) {
        token = jindo.$S(aFontFamily[h].replace(/['"]/gi, "")).trim().$value();
        if (!token || aGeneric[token]) {
          continue;
        }
        if (token == sPrimary) {
  ...
  _getFontFamilyFromLI: function (elLi) {
    ...
    var em = elLi.getElementsByTagName("EM")[0];
    if (!em) {
      return "";
    }
    return em.style.fontFamily || "";
  },

  _getFontNameLabelFromLI: function (elLi) {
    var btn, span, n, s;
    if (!elLi || !elLi.firstChild) {
      return this.sDefaultText;
    }
    btn = elLi.firstChild;
    if (btn.nodeType !== 1 || !btn.firstChild) {
      return this.sDefaultText;
    }
    span = btn.firstChild;
    if (span.nodeType === 3) {
      return span.nodeValue || this.sDefaultText;
    }
    if (span.nodeType === 1) {
      n = span.firstChild;
      if (n && n.nodeType === 3) {
        return n.nodeValue || this.sDefaultText;
      }
      s = span.textContent || span.innerText || "";
      if (s) {
        return jindo.$S(s.split("(")[0]).trim().$value() || this.sDefaultText;
      }
    }
    return this.sDefaultText;
  },
...
*/
    ...
    if (
      this.htBrowser.ie &&
      this.htBrowser.nativeVersion >= 9 &&
    ...
    sSampleText =
      sSampleText || this.oApp.$MSG("SE2M_FontNameWithLayerUI.sSampleText");
...
nhn.ColorPicker = jindo
  .$Class({
    ...
    $init: function (oElement, oOptions) {
      ...
      this.canvasType = jindo.$(oElement).filters
        ? "Filter"
        : jindo.$("<canvas>").getContext
          ? "Canvas"
          : null;
    ...
    _paintColWithFilter: function () {
      ...
      jindo
        ...
        .css({
          ...
          height: "100%",
          filter:
            "progid:DXImageTransform.Microsoft.Gradient(GradientType=1,StartColorStr='#FFFFFFFF',EndColorStr='#00FFFFFF')",
        })
        .appendTo(this.elem);
      ...
      jindo
        ...
        .css({
          ...
          height: "100%",
          filter:
            "progid:DXImageTransform.Microsoft.Gradient(GradientType=0,StartColorStr='#00000000',EndColorStr='#FF000000')",
        })
        .appendTo(this.elem);
    },
    _paintColWithCanvas: function () {
      ...
      var ctx = cvs
        .attr("width", cvs.width())
        .attr("height", cvs.height())
        .$value()
        .getContext("2d");
      var lin = null;
      var w = cvs.width();
    ...
    _paintOneWithFilter: function () {
      ...
      jindo
        ...
        .css({
          ...
          height: "100%",
          filter:
            "progid:DXImageTransform.Microsoft.Gradient(GradientType=1,StartColorStr='#00000000',EndColorStr='#FF000000')",
        })
        .appendTo(this.elem);
    },
    ...
    _paintHueWithFilter: function () {
      ...
      for (var i = 1; i < 7; i++) {
        ...
        elDiv
          .css(vert ? "left" : "top", 0)
          .css(vert ? "width" : "height", "100%");
        elDiv
          .css(vert ? "top" : "left", sp + "px")
          .css(vert ? "height" : "width", width + "px");
      }
    },
    _paintHueWithCanvas: function () {
      ...
      var ctx = cvs
        ...
        .$value()
        .getContext("2d");
      var lin = ctx.createLinearGradient(
        0,
        0,
        vtc ? 0 : cvs.width(),
        vtc ? cvs.height() : 0,
...
nhn.husky.SE2M_Accessibility = jindo.$Class({
  ...
  _assignHTMLElements: function (elAppContainer) {
    this.elHelpPopupLayer = jindo.$$.getSingle(
      "DIV.se2_accessibility",
      elAppContainer,
    );
    this.welHelpPopupLayer = jindo.$Element(this.elHelpPopupLayer);

    //close buttons
    this.oCloseButton = jindo.$$.getSingle(
      "BUTTON.se2_close",
      this.elHelpPopupLayer,
    );
    this.oCloseButton2 = jindo.$$.getSingle(
      "BUTTON.se2_close2",
      this.elHelpPopupLayer,
    );
  ...
  $ON_MSG_APP_READY: function () {
    ...
    if (
      (this.sType == "basic" || this.sType == "light") &&
      this.sLang != "ko_KR"
    ) {
      //do nothing
      return;
    ...
    if (this.htAccessOption.sTitleElementId) {
      this.oApp.registerBrowserEvent(
        document.getElementById(this.htAccessOption.sTitleElementId),
        "keydown",
        "MOVE_TO_EDITAREA",
        [],
      );
  ...
  $LOCAL_BEFORE_FIRST: function (sMsg) {
    jindo
      .$Fn(
        jindo
          .$Fn(this.oApp.exec, this.oApp)
          .bind("CLOSE_HELP_POPUP", [this.oCloseButton]),
        this,
      )
      .attach(this.oCloseButton, "click");
    jindo
      .$Fn(
        jindo
          .$Fn(this.oApp.exec, this.oApp)
          .bind("CLOSE_HELP_POPUP", [this.oCloseButton2]),
        this,
      )
      .attach(this.oCloseButton2, "click");
  ...
  $ON_FOCUS_NEXT_ELEMENT: function () {
    ...
    if (this.htAccessOption.sNextElementId) {
      this._currentNextFocusElement = document.getElementById(
        this.htAccessOption.sNextElementId,
      );
    } else {
      this._currentNextFocusElement = this._findNextFocusElement(
        this.elAppContainer,
      );
    }
   ...
   * */
  _recursivePreorderTraversalFilter: function (
    node,
    filterFunction,
    isReversed,
  ) {
    ...
    if (_bStopFindingNextElement) {
      ...
      if (isReversed) {
        for (var len = node.childNodes.length, i = len - 1; i >= 0; i--) {
          self._recursivePreorderTraversalFilter(
            node.childNodes[i],
            filterFunction,
            true,
          );
  ...
  _isFocusTag: function () {
    ...
    for (var i = 0, len = aFocusTagViaTabKey.length; i < len; i++) {
      if (
        self.nodeType === 1 &&
        self.nodeName &&
        self.nodeName.toUpperCase() == aFocusTagViaTabKey[i] &&
        !self.disabled &&
        jindo.$Element(self).visible()
      ) {
  ...
  $ON_FOCUS_BEFORE_ELEMENT: function () {
    ...
    if (this.htAccessOption.sBeforeElementId) {
      this._currentPreviousFocusElement = document.getElementById(
        this.htAccessOption.sBeforeElementId,
      );
    } else {
      this._currentPreviousFocusElement = this._findPreviousFocusElement(
        this.elAppContainer,
      ); // 삽입될 대상
    }
  ...
  $ON_FOCUS_TOOLBAR_AREA: function () {
    this.oButton = jindo.$$.getSingle(
      "BUTTON.se2_font_family",
      this.elAppContainer,
    );
    if (this.oButton && !this.oButton.disabled) {
      // [SMARTEDITORSUS-1369] IE9이하에서 disabled 요소에 포커스를 주면 오류 발생
      window.focus();
      this.oButton.focus();
    }
  },
...
nhn.husky.SE2M_Quote = jindo.$Class({
  ...
  $init: function () {
    ...
    this.htQuoteStyles_view["se2_quote1"] =
      "_zoom:1;padding:0 8px; margin:0 0 30px 20px; margin-right:15px; border-left:2px solid #cccccc;color:#888888;";
    this.htQuoteStyles_view["se2_quote2"] =
      "_zoom:1;margin:0 0 30px 13px;padding:0 8px 0 16px;background:url(" +
      sImageBaseURL +
      "/bg_quote2.gif) 0 3px no-repeat;color:#888888;";
    this.htQuoteStyles_view["se2_quote3"] =
      "_zoom:1;margin:0 0 30px 0;padding:10px;border:1px dashed #cccccc;color:#888888;";
    this.htQuoteStyles_view["se2_quote4"] =
      "_zoom:1;margin:0 0 30px 0;padding:10px;border:1px dashed #66b246;color:#888888;";
    this.htQuoteStyles_view["se2_quote5"] =
      "_zoom:1;margin:0 0 30px 0;padding:10px;border:1px dashed #cccccc;background:url(" +
      sImageBaseURL +
    ...
    this.htQuoteStyles_view["se2_quote6"] =
      "_zoom:1;margin:0 0 30px 0;padding:10px;border:1px solid #e5e5e5;color:#888888;";
    this.htQuoteStyles_view["se2_quote7"] =
      "_zoom:1;margin:0 0 30px 0;padding:10px;border:1px solid #66b246;color:#888888;";
    this.htQuoteStyles_view["se2_quote8"] =
      "_zoom:1;margin:0 0 30px 0;padding:10px;border:1px solid #e5e5e5;background:url(" +
      sImageBaseURL +
      "/bg_b1.png) repeat;_background:none;_filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src='" +
      sImageBaseURL +
      "/bg_b1.png',sizingMethod='scale');color:#888888;";
    this.htQuoteStyles_view["se2_quote9"] =
      "_zoom:1;margin:0 0 30px 0;padding:10px;border:2px solid #e5e5e5;color:#888888;";
    this.htQuoteStyles_view["se2_quote10"] =
      "_zoom:1;margin:0 0 30px 0;padding:10px;border:2px solid #e5e5e5;background:url(" +
      sImageBaseURL +
      "/bg_b1.png) repeat;_background:none;_filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src='" +
      sImageBaseURL +
      "/bg_b1.png',sizingMethod='scale');color:#888888;";
  ...
  _isBlankQuote: function (elParentQuote) {
    var elChild,
      ...
      isBlank = function (oNode) {
        ...
        if (
          (oNode.tagName === "P" || oNode.tagName === "SPAN") &&
    ...
    if (
      isBlankText(elParentQuote.innerHTML) ||
      elParentQuote.innerHTML === "<br>"
    ) {
      return true;
    }

    if (bChrome || bSafari) {
      // [SMARTEDITORSUS-352], [SMARTEDITORSUS-502]
...
nhn.husky.SE2M_TableCreator = jindo.$Class({
  ...
  $ON_MSG_APP_READY: function () {
    this.sReEditGuideMsg_table = this.oApp.$MSG(
      "SE2M_ReEditAction.reEditGuideMsg.table",
    );
    this.oApp.exec("REGISTER_UI_EVENT", [
      "table",
      "click",
  ...
  irToDbDOM: function (oTmpNode) {
    ...
    for (var i = 0, nCount = aNoBorderTable.length; i < nCount; i++) {
      ...
      jindo
        .$Element(oTable)
        .css({ border: "", borderLeft: "", borderBottom: "" });
  ...
  dbToIrDOM: function (oTmpNode) {
    ...
    for (var i = 0, nCount = aNoBorderTable.length; i < nCount; i++) {
      ...
      jindo
        .$Element(oTable)
        .css({ border: "1px dashed #c7c7c7", borderLeft: 0, borderBottom: 0 });
      ...
      jindo.$A(aTDs).forEach(function (oTD, nIdx, oTDArray) {
        jindo
          .$Element(oTD)
          .css({ border: "1px dashed #c7c7c7", borderTop: 0, borderRight: 0 });
      });
    }
  },
});
...
nhn.husky.SE2M_StyleRemover = jindo.$Class({
  ...
  $ON_REMOVE_STYLE: function (oSelection) {
    ...
    for (var i = 0; i < aNodes.length; i++) {
      ...
      if (
        ...
        if (
          jindo.$Element(oNode).css("display") == "inline" &&
          oNode.tagName != "IMG" &&
          oNode.tagName != "IFRAME" &&
          (!oNode.firstChild || oSelection._isBlankTextNode(oNode.firstChild))
        ) {
          oNode.parentNode.removeChild(oNode);
        }
      }
    ...
    // remove start marker
    var oMarker = this._document.getElementById(
      oSelection.HUSKY_BOOMARK_START_ID_PREFIX + sBookmarkID,
    );
    while (oMarker) {
      ...
      while (
        oParent &&
        (!oParent.firstChild ||
          (!oParent.firstChild.nextSibling &&
            oSelection._isBlankTextNode(oParent.firstChild)))
      ) {
        var oNextParent = oParent.parentNode;
        oParent.parentNode.removeChild(oParent);
        oParent = oNextParent;
      }
      oMarker = this._document.getElementById(
        oSelection.HUSKY_BOOMARK_START_ID_PREFIX + sBookmarkID,
      );
    }
    ...
    // remove end marker
    var oMarker = this._document.getElementById(
      oSelection.HUSKY_BOOMARK_END_ID_PREFIX + sBookmarkID,
    );
    while (oMarker) {
      oParent = nhn.DOMFix.parentNode(oMarker);
      oParent.removeChild(oMarker);
  ...
  $ON_REMOVE_STYLE_AND_PASTE_HTML: function (sHtml, bNoUndo) {
    var htBrowser, elDivHolder, elFirstTD, aNodesInSelection, oSelection;
    ...
    for (var i = 0; i < aNodesInSelection.length; i++) {
      if (
        !!aNodesInSelection[i].tagName &&
        aNodesInSelection[i].tagName.toLowerCase() == "td"
      ) {
  ...
  _removeStyle: function (aNodes) {
    ...
    for (var i = 0; i < aNodes.length; i++) {
      ...
      if (oNode.firstChild) {
        if (
          arNodes.indexOf(this.oHuskyRange._getVeryLastRealChild(oNode)) == -1
        ) {
          continue;
        }
        if (
          arNodes.indexOf(this.oHuskyRange._getVeryFirstRealChild(oNode)) == -1
        ) {
          continue;
        }
      }
...
nhn.husky.SE2M_TableEditor = jindo.$Class({
          ...
          for (var n = 0; n < aCellType.length; n++) {
            ...
            for (var i = 0, nLen = aCells.length; i < nLen; i++) {
              if (aCells[i].className) {
                aCells[i].className = aCells[i].className.replace(
                  this.CELL_SELECTION_CLASS,
                  "",
                );
                if (aCells[i].getAttribute(this.TMP_BGC_ATTR)) {
                  aCells[i].style.backgroundColor = aCells[i].getAttribute(
                    this.TMP_BGC_ATTR,
                  );
                  aCells[i].removeAttribute(this.TMP_BGC_ATTR);
                } else if (aCells[i].getAttribute(this.TMP_BGIMG_ATTR)) {
                  jindo
                    .$Element(this.aCells[i])
                    .css(
                      "backgroundImage",
                      aCells[i].getAttribute(this.TMP_BGIMG_ATTR),
                    );
  ...
  $ON_MSG_APP_READY: function () {
    this.oApp.registerLazyMessage(
      ["EVENT_EDITING_AREA_MOUSEMOVE", "STYLE_TABLE"],
      ["hp_SE2M_TableEditor$Lazy.js", "SE2M_TableTemplate.js"],
    );
  },
});
/**
...
nhn.husky.SE2M_QuickEditor_Common = jindo.$Class({
  ...
  $ON_MSG_APP_READY: function () {
    ...
    if (htConfiguration) {
      this._bUseConfig =
        !!htConfiguration.common &&
        typeof htConfiguration.common.bUseConfig !== "undefined"
          ? htConfiguration.common.bUseConfig
          : true;
    }
...
Shortcut.Store = {
  ...
  set: function (sId, sKey, oElement) {
    ...
    this.datas[sId] = idData
      ? idData.createKey(sKey)
      : new Shortcut.Data(sId, sKey, oElement);
  },
...
Shortcut.Helper = {
  keyInterpretor: function (sKey) {
    ...
    if (key) {
      var sKey =
        Shortcut.Store.anthorKeyHash[key.toUpperCase()] ||
...
nhn.husky.SE_UndoRedo = jindo.$Class({
  ...
  $ON_UNDO: function () {
    ...
    var oUndoCallback =
      this.aUndoHistory[this.oCurStateIdx.nIdx].oUndoCallback[
        this.oCurStateIdx.nStep
      ];
    var sCurrentPath =
      this.aUndoHistory[this.oCurStateIdx.nIdx].sParentXPath[
        this.oCurStateIdx.nStep
      ];
    ...
    if (
      sUndoHistoryPath !== sCurrentPath &&
      sUndoHistoryPath.indexOf(sCurrentPath) === 0
    ) {
      // 현재의 Path가 Undo의 Path보다 범위가 큰 경우
      bUseDefault = true;
    }
  ...
  $ON_REDO: function () {
    ...
    if (
      this.oCurStateIdx.nIdx == this.aUndoHistory.length - 1 &&
    ...
    var oRedoCallback =
      this.aUndoHistory[this.oCurStateIdx.nIdx].oRedoCallback[
        this.oCurStateIdx.nStep
      ];
  ...
  $ON_RESTORE_UNDO_HISTORY: function (nUndoIdx, nUndoStateStep, bUseDefault) {
    ...
    if (oBookmark && oBookmark.sType == "scroll") {
      setTimeout(
        jindo
          .$Fn(function () {
            this.oApp.getWYSIWYGDocument().documentElement.scrollTop =
              oBookmark.nScrollTop;
          }, this)
          .bind(),
        0,
      );
  ...
  _doRecordUndoHistory: function (sAction, htRecordOption) {
    ...
    if (elSaveTarget) {
      ...
      sParentXPath = this._getParentXPath(
        oBookmark,
        bMustBlockElement,
        bMustBlockContainer,
      );
    ...
    if (sParentXPath === this.sDefaultXPath) {
      ...
      sCurContent = oSelection._evaluateXPath(
        sParentXPath,
        oSelection._document,
      ).innerHTML;
    }
  ...
  $ON_DO_RECORD_UNDO_HISTORY_AT: function (
    oInsertionIdx,
    sAction,
    sContent,
    sFullContents,
  ...
  _doRecordUndoHistoryAt: function (
    oInsertionIdx,
    sAction,
    sContent,
    sFullContents,
    ...
    if (oInsertionIdx.nStep !== 0) {
      this.aUndoHistory[oInsertionIdx.nIdx].nTotalSteps =
        oInsertionIdx.nStep + 1;
      this.aUndoHistory[oInsertionIdx.nIdx].sContent[oInsertionIdx.nStep] =
        sContent;
      this.aUndoHistory[oInsertionIdx.nIdx].sFullContents[oInsertionIdx.nStep] =
        sFullContents;
      this.aUndoHistory[oInsertionIdx.nIdx].oBookmark[oInsertionIdx.nStep] =
        oBookmark;
      this.aUndoHistory[oInsertionIdx.nIdx].sParentXPath[oInsertionIdx.nStep] =
        sParentXPath;
      this.aUndoHistory[oInsertionIdx.nIdx].oUndoCallback[oInsertionIdx.nStep] =
        oUndoCallback;
      this.aUndoHistory[oInsertionIdx.nIdx].oRedoCallback[oInsertionIdx.nStep] =
        oRedoCallback;
    } else {
      ...
      this.aUndoHistory.splice(
        oInsertionIdx.nIdx,
        this._historyLength - oInsertionIdx.nIdx,
        oNewHistory,
      );
      this._historyLength = this.aUndoHistory.length;
    }
  ...
  _getTargetXPath: function (oBookmark, sSaveTarget) {
    ...
    var sParentXPath = this.sDefaultXPath,
      ...
      aParentPath = [],
      nPathLen =
        aStartXPath.length < aEndXPath.length
          ? aStartXPath.length
          : aEndXPath.length,
      nPathIdx = 0,
      nTargetIdx = -1;
    ...
    for (nPathIdx = 0; nPathIdx < nPathLen; nPathIdx++) {
      ...
      if (
        aStartXPath[nPathIdx] === "" ||
        aStartXPath[nPathIdx] === "HTML" ||
        aStartXPath[nPathIdx] === "BODY"
      ) {
        continue;
      }
  ...
  _getParentXPath: function (
    ...
    if (
      oBookmark[0].sXPath === sParentXPath ||
      oBookmark[1].sXPath === sParentXPath
    ) {
      return sParentXPath;
    }
    ...
    nSnapShotLen =
      aSnapShotStart.length < aSnapShotEnd.length
        ? aSnapShotStart.length
        : aSnapShotEnd.length;
    nPathLen =
      aStartXPath.length < aEndXPath.length
        ? aStartXPath.length
...
nhn.husky.Utils = jindo.$Class({
  ...
  $ON_ATTACH_HOVER_EVENTS: function (aElms, htOptions) {
    ...
    var wfRemoveClass = jindo.$Fn(function (wev) {
      jindo
        .$Element(fnElmToTarget(wev.currentElement))
        .removeClass(sHoverClass);
    }, this);
...
nhn.husky.ActiveLayerManager = jindo.$Class({
  ...
  $ON_TOGGLE_ACTIVE_LAYER: function (
    oLayer,
    sOnOpenCmd,
    aOnOpenParam,
    sOnCloseCmd,
...
nhn.husky.StringConverterManager = jindo.$Class({
  ...
  addConverter: function (sRuleName, funcConverter) {
    ...
    this.oConverters[sRuleName][this.oConverters[sRuleName].length] =
      funcConverter;
  },
  ...
  addConverter_DOM: function (sRuleName, funcConverter) {
    ...
    this.oConverters_DOM[sRuleName][this.oConverters_DOM[sRuleName].length] =
      funcConverter;
  },
});
//}
...
nhn.husky.LazyLoader = jindo.$Class({
  ...
  $ON_MSG_APP_READY: function () {
    for (var i = 0; i < this.aToDo.length; i++) {
      ...
      this._createBeforeHandlersAndSaveURLInfo(
        htToDoDetail.oMsgs,
        htToDoDetail.sURL,
        htToDoDetail.elTarget,
        htToDoDetail.htOptions,
      );
    }
  },
  ...
  _createBeforeHandlersAndSaveURLInfo: function (
    oMsgs,
    sURL,
    elTarget,
    htOptions,
  ...
  if (this._instance == null) {
    this._instance = new (function () {
      ...
      this.openWindow = function (oWinOpt) {
        ...
        if (op.oApp == null) {
          alert(
            "팝업 요청시 옵션으로 oApp(허스키 reference) 값을 설정하셔야 합니다.",
          );
        }
        ...
        this._whtPluginWin.add(
          this.getCorrectKey(this._whtPlugin, op.oApp),
          win,
        );
...
nhn.husky.PopUpManager.setCallback = function (oOpenWin, sMsg, oData) {
  if (this._instance.getPluginWin().hasValue(oOpenWin)) {
    var key = this._instance.getCorrectKey(
      this._instance.getPluginWin(),
      oOpenWin,
    );
    if (key) {
...
nhn.husky.SE2M_UtilPlugin = jindo.$Class({
  ...
  $ON_SE2_ATTACH_HOVER_EVENTS: function (aElms) {
    this.oApp.exec("ATTACH_HOVER_EVENTS", [
      aElms,
      { fnElmToSrc: this._elm2Src, fnElmToTarget: this._elm2Target },
    ]);
  },
...
nhn.husky.SE2M_Utils = {
  ...
  getCustomCSS: function (sStr, rxValue, sDivision) {
    var ret = [];
    if (
      "undefined" == typeof sStr ||
      "undefined" == typeof rxValue ||
      !sStr ||
      !rxValue
    ) {
      return ret;
...
ellipsisElementsToDesinatedWidth([jindo.$("c"), jindo.$("b"), jindo.$("a")], "...", [100, 50, 50], function(){
	 ...
	 */
  ellipsisElementsToDesinatedWidth: function (
    aElement,
    sStringTail,
    aMinWidth,
    fCondition,
    ...
    jindo.$A(aElement).forEach(function (el, i) {
      ...
      nhn.husky.SE2M_Utils.ellipsisByPixel(
        el,
        sStringTail,
        aMinWidth[i],
        fCondition,
...
==> { a : 1, c : 3 }
  ...
  isBlankNode: function (elNode) {
    ...
    var bEmptyP = function (elNode) {
      ...
      if (elNode.tagName == "P") {
        ...
        if (elNode.childNodes.length == 1) {
          if (
            elNode.firstChild.tagName == "IMG" ||
            elNode.firstChild.tagName == "IFRAME"
          ) {
            return false;
          }
  ...
  isFirstChildOfNode: function (sTagName, sParentTagName, elNode) {
    ...
    if (
      elNode.tagName == sParentTagName &&
      elNode.firstChild.tagName == sTagName
    ) {
      return true;
    }
  ...
  clone: function (oSrc, oChange) {
    if (
      "undefined" != typeof oSrc &&
      !!oSrc &&
      (oSrc.constructor == Array || oSrc.constructor == Object)
    ) {
  ...
  iframeAlignConverter: function (el, oDoc) {
    ...
    if (sTagName == "DIV" || sTagName == "P") {
      ...
      //현재 align을 얻어오기.
      var sAlign =
        jindo.$Element(el).attr("align") ||
        jindo.$Element(el).css("text-align");
  ...
  getJsonDatafromXML: function (sXML) {
    ...
    var cb = function ($0, $1, $2, $3, $4, $5) {
      ...
      if (typeof $1 == "string" && $1) {
        ...
        // [SMARTEDITORSUS-1525] 닫는 태그인데 공백문자가 들어있어 cdata 값을 덮어쓰는 경우 방지
        cdata = es($5);
      }
...
nhn.husky.AutoResizer = jindo.$Class({
  ...
  bind: function () {
    this.welCloneDiv = jindo.$Element(
      this.welHiddenDiv.$value().cloneNode(false),
    );
...
(function () {
  ...
  IsInstalledFont = function (sFont) {
    ...
    var sStyle =
      "position:absolute !important; font-size:200px !important; left:-9999px !important; top:-9999px !important;";
...
nhn.husky.SE2B_CSSLoader = jindo.$Class({
  ...
  $init: function () {
    ...
    if (!jindo.$Agent().navigator().ie) {
      ...
      for (var i = 0, nLen = this.aInstantLoadTrigger.length; i < nLen; i++) {
        this["$BEFORE_" + this.aInstantLoadTrigger[i]] = jindo
          .$Fn(function () {
            this.loadSE2CSS();
          }, this)
          .bind();
      ...
      for (var i = 0, nLen = this.aDelayedLoadTrigger.length; i < nLen; i++) {
        ...
        this["$BEFORE_" + this.aDelayedLoadTrigger[i]] = jindo
          .$Fn(function (sMsg) {
  ...
  loadSE2CSS: function (sMsg, oArgs) {
    ...
    nhn.husky.SE2M_Utils.loadCSS(
      this.htOptions.sCSSBaseURI + "/smart_editor2_items.css",
      fnCallback,
    );
...
var oMessageMap = {
  ...
  "SE_FindReplace.notSupportedBrowser":
    "현재 사용하고 계신 브라우저에서는 사용하실수 없는 기능입니다.\n\n이용에 불편을 드려 죄송합니다.",
  ...
  "SE.failedToLoadFlash":
    "플래시가 차단되어 있어 해당 기능을 사용할 수 없습니다.",
  "SE2M_EditingModeChanger.confirmTextMode":
    "텍스트 모드로 전환하면 작성된 내용은 유지되나, \n\n글꼴 등의 편집효과와 이미지 등의 첨부내용이 모두 사라지게 됩니다.\n\n전환하시겠습니까?",
...
nhn.husky.SE_OuterIFrameControl = $Class({
  ...
  _assignHTMLObjects: function (oAppContainer) {
    ...
    this.oResizeGrip = cssquery.getSingle(
      ".husky_seditor_editingArea_verticalResizer",
      oAppContainer,
    );
  ...
  _keydown: function (oEvent) {
    ...
    if (oKeyInfo.keyCode >= 33 && oKeyInfo.keyCode <= 40) {
      this.oApp.exec("MSG_EDITING_AREA_RESIZE_STARTED", []);
      this.oApp.exec("RESIZE_EDITING_AREA_BY", [
        0,
        this.aHeightChangeKeyMap[oKeyInfo.keyCode - 33],
      ]);
      this.oApp.exec("MSG_EDITING_AREA_RESIZE_ENDED", []);
  ...
  _mousemove_parent: function (oEvent) {
    var iHeightChange =
      oEvent.pos().pageY - (this.welIFrame.offset().top + this.iStartHeight);
    this.oApp.exec("RESIZE_EDITING_AREA_BY", [0, iHeightChange]);
  },
...
nhn.husky.SE_ToolbarToggler = $Class({
  ...
  $ON_SE_TOGGLE_TOOLBAR: function () {
    this.toolbarArea.style.display =
      this.toolbarArea.style.display == "none" ? "block" : "none";
...
nhn.husky.HuskyCore.mixin(nhn.husky.SE2M_FindReplacePlugin, {
  ...
  _assignHTMLElements: function () {
    ...
    this.elDropdownLayer = jindo.$$.getSingle(
      "DIV.husky_se2m_findAndReplace_layer",
      oAppContainer,
    );
    ...
    this.oFindInputSet = jindo.$$.getSingle(
      ".husky_se2m_find_ui",
      this.elDropdownLayer,
    );
    this.oReplaceInputSet = jindo.$$.getSingle(
      ".husky_se2m_replace_ui",
      this.elDropdownLayer,
    );
    ...
    this.oFindNextButton = jindo.$$.getSingle(
      "BUTTON.husky_se2m_find_next",
      this.elDropdownLayer,
    );

    this.oReplaceFindNextButton = jindo.$$.getSingle(
      "BUTTON.husky_se2m_replace_find_next",
      this.elDropdownLayer,
    );

    this.oReplaceButton = jindo.$$.getSingle(
      "BUTTON.husky_se2m_replace",
      this.elDropdownLayer,
    );
    this.oReplaceAllButton = jindo.$$.getSingle(
      "BUTTON.husky_se2m_replace_all",
      this.elDropdownLayer,
    );

    this.aCloseButtons = jindo.$$(
      "BUTTON.husky_se2m_cancel",
      this.elDropdownLayer,
    );
  },
  ...
  $LOCAL_BEFORE_FIRST: function (sMsg) {
    ...
    for (var i = 0; i < this.aCloseButtons.length; i++) {
      ...
      var func = jindo
        .$Fn(this.oApp.exec, this.oApp)
        .bind("HIDE_FIND_REPLACE_LAYER", [this.elDropdownLayer]);
      jindo.$Fn(func, this).attach(this.aCloseButtons[i], "click");
    }

    jindo
      .$Fn(jindo.$Fn(this.oApp.exec, this.oApp).bind("SHOW_FIND", []), this)
      .attach(this.oFindTab, "click");
    jindo
      .$Fn(jindo.$Fn(this.oApp.exec, this.oApp).bind("SHOW_REPLACE", []), this)
      .attach(this.oReplaceTab, "click");

    jindo
      .$Fn(jindo.$Fn(this.oApp.exec, this.oApp).bind("FIND", []), this)
    ...
    jindo
      .$Fn(jindo.$Fn(this.oApp.exec, this.oApp).bind("REPLACE", []), this)
      .attach(this.oReplaceButton, "click");
    jindo
      .$Fn(jindo.$Fn(this.oApp.exec, this.oApp).bind("REPLACE_ALL", []), this)
  ...
  $ON_SHOW_FIND_REPLACE_LAYER: function () {
    ...
    this.oApp.exec("SHOW_DIALOG_LAYER", [
      this.elDropdownLayer,
      {
        ...
        fnOnDragStart: jindo
          .$Fn(this.oApp.exec, this.oApp)
          .bind("SHOW_EDITING_AREA_COVER"),
        fnOnDragEnd: jindo
          .$Fn(this.oApp.exec, this.oApp)
          .bind("HIDE_EDITING_AREA_COVER"),
  ...
  $ON_REPLACE: function () {
    ...
    var iReplaceResult = this.oFindReplace.replace(
      sOriginal,
      sReplacement,
      false,
    );
  ...
  $ON_REPLACE_ALL: function () {
    ...
    var iReplaceAllResult = this.oFindReplace.replaceAll(
      sOriginal,
      sReplacement,
      false,
    );
    ...
    if (iReplaceAllResult === 0) {
      ...
      if (iReplaceAllResult < 0) {
        ...
        alert(
          this.oApp.$MSG("SE_FindReplace.replaceAllResultP1") +
            iReplaceAllResult +
...
nhn.husky.HuskyCore.mixin(nhn.husky.SE2M_Quote, {
  ...
  $ON_EVENT_SE2_BLOCKQUOTE_LAYER_CLICK: function (weEvent) {
    var elButton = nhn.husky.SE2M_Utils.findAncestorByTagName(
      "BUTTON",
      weEvent.element,
    );
  ...
  _unwrapBlock: function (tag) {
    ...
    while (elCommonAncestor.firstChild) {
      elCommonAncestor.parentNode.insertBefore(
        elCommonAncestor.firstChild,
        elCommonAncestor,
      );
    }
  ...
  _wrapBlock: function (tag, className) {
    ...
    if (
      oSelection.startContainer === oSelection.endContainer &&
      ...
      if (
        ...
        nhn.husky.SE2M_Utils.isFirstChildOfNode(
          "IMG",
          oSelection.startContainer.tagName,
    ...
    if (
      oSelection.startContainer == oSelection.endContainer &&
      oSelection.endOffset - oSelection.startOffset == 1
    ) {
      elCommonNode =
        oSelection.startContainer.childNodes[oSelection.startOffset];
    } else {
    ...
    while (
      !elCommonAncestor.tagName ||
      (elCommonAncestor.tagName &&
        elCommonAncestor.tagName.match(/UL|OL|LI|IMG|IFRAME/))
    ) {
      elCommonAncestor = elCommonAncestor.parentNode;
    }
    ...
    while (
      oNode &&
      oNode != elCommonAncestor &&
      oNode.parentNode != elCommonAncestor
    ) {
      oNode = oNode.parentNode;
    }
    ...
    if (
      oFormattingNode &&
      oFormattingNode.parentNode &&
      oFormattingNode.parentNode.tagName == "BODY" &&
...
nhn.husky.HuskyCore.mixin(nhn.husky.SE2M_SCharacter, {
  ...
  $LOCAL_BEFORE_FIRST: function (sFullMsg) {
    ...
    var funcInsert = jindo
      .$Fn(this.oApp.exec, this.oApp)
      .bind("INSERT_SCHARACTERS", [this.oTextField.value]);
...
nhn.husky.HuskyCore.mixin(nhn.husky.SE2M_TableCreator, {
  ...
  $LOCAL_BEFORE_FIRST: function () {
    ...
    this.oApp.registerBrowserEvent(
      this.elText_col,
      "change",
      "TABLE_SET_COLUMN_NUM",
      [null, 0],
    );
    this.oApp.registerBrowserEvent(
      this.elText_borderSize,
      "change",
      "TABLE_SET_BORDER_SIZE",
      [null, 0],
    );
    ...
    jindo
      .$Fn(this._numRowKeydown, this)
      .attach(this.elText_row.parentNode, "keydown");
    ...
    jindo
      .$Fn(this._numColKeydown, this)
      .attach(this.elText_col.parentNode, "keydown");

    this.oApp.registerBrowserEvent(
      this.elBtn_incBorderSize,
      "click",
      "TABLE_INC_BORDER_SIZE",
    );
    this.oApp.registerBrowserEvent(
      this.elBtn_decBorderSize,
      "click",
      "TABLE_DEC_BORDER_SIZE",
    );
    jindo
      .$Fn(this._borderSizeKeydown, this)
      .attach(this.elText_borderSize.parentNode, "keydown");

    this.oApp.registerBrowserEvent(
      this.elBtn_borderStyle,
      "click",
      "TABLE_TOGGLE_BORDER_STYLE_LAYER",
    );
    this.oApp.registerBrowserEvent(
      this.elBtn_tableStyle,
      "click",
      "TABLE_TOGGLE_STYLE_LAYER",
    );

    this.oApp.registerBrowserEvent(
      this.elBtn_borderColor,
      "click",
    ...
    this.oApp.registerBrowserEvent(
      this.elRadio_manualStyle,
      "click",
      "TABLE_ENABLE_MANUAL_STYLE",
    );
    this.oApp.registerBrowserEvent(
      this.elRadio_templateStyle,
      "click",
    ...
    this.oApp.exec("TABLE_SET_BORDER_COLOR", [
      /#[0-9A-Fa-f]{6}/.test(this.elText_borderColor.value)
        ? this.elText_borderColor.value
        : "#cccccc",
    ]);
    this.oApp.exec("TABLE_SET_BGCOLOR", [
      /#[0-9A-Fa-f]{6}/.test(this.elText_BGColor.value)
        ? this.elText_BGColor.value
        : "#ffffff",
    ]);
  ...
  _divideParagraph: function (oSelection) {
    ...
    var _htAncestor_P = nhn.husky.SE2M_Utils.findAncestorByTagNameWithCount(
      "P",
      _elCommonAncestorContainer,
    );
    var _elAncestor_P = _htAncestor_P.elNode;
    if (_elAncestor_P) {
      var _htAncestor_Cell =
        nhn.husky.SE2M_Utils.findClosestAncestorAmongTagNamesWithCount(
          ["TD", "TH"],
          _elCommonAncestorContainer,
        );
      ...
      if (_elAncestor_Cell) {
        ...
        // selection을 기준으로 cell이 발견되지 않으면 그대로 진행
        oParentP = _elAncestor_P;
      }
    }
  ...
  $ON_TABLE_SET_BORDER_SIZE: function (nBorderWidth, nBorderWidthDiff) {
    nBorderWidth =
      nBorderWidth || parseInt(this.elText_borderSize.value, 10) || 0;
  ...
  _showNewTable: function () {
    ...
    this.elTable_preview.parentNode.insertBefore(
      oNewTable,
  ...
  _doGetTableString: function (
    ...
    var sTableStyle = this.aTableStyleByBorder[nBorderStyleIdx]
      .replace(/#BorderSize#/g, this.nBorderSize)
    ...
    var sTDStyle = this.aTDStyleByBorder[nBorderStyleIdx]
      .replace(/#BorderSize#/g, this.nBorderSize)
      .replace(/#BorderSize\*([0-9]+)#/g, function (sAll, s1) {
        return nBorderSize * parseInt(s1, 10);
      })
    ...
    var sTempNoBorderClass =
      nBorderStyleIdx == 1 ? 'attr_no_border_tbl="1"' : "";
...
nhn.husky.HuskyCore.mixin(nhn.husky.SE2M_TableEditor, {
  ...
  _assignHTMLObjects: function () {
    ...
    this.elQELayer = jindo.$$.getSingle(
      "DIV.q_table_wrap",
      this.oApp.htOptions.elAppContainer,
    );
    ...
    this.welPanelTableTemplateArea = jindo.$Element(
      this.elPanelTableTemplateArea,
    );
    ...
    this.elInputRadioBGColor = jindo.$$.getSingle(
      "INPUT.husky_se2m_radio_bgc",
      this.elQELayer,
    );
    this.elInputRadioBGImg = jindo.$$.getSingle(
      "INPUT.husky_se2m_radio_bgimg",
      this.elQELayer,
    );
  ...
  $LOCAL_BEFORE_FIRST: function (sMsg) {
    ...
    this.nPageLeftRightMargin =
      parseInt(elBody.css("marginLeft"), 10) +
      parseInt(elBody.css("marginRight"), 10);
    this.nPageTopBottomMargin =
      parseInt(elBody.css("marginTop"), 10) +
      parseInt(elBody.css("marginBottom"), 10);
...
*/
    ...
    this.wfnMousedown_ResizeCover = jindo.$Fn(
      this._mousedown_ResizeCover,
      this,
    );
    this.wfnMousemove_ResizeCover = jindo.$Fn(
      this._mousemove_ResizeCover,
      this,
    );
    ...
    this.oApp.registerBrowserEvent(
      this.elBtnAddColumnRight,
      "click",
      "TE_INSERT_COLUMN_RIGHT",
    );

    this.oApp.registerBrowserEvent(
      this.elBtnAddRowBelow,
      "click",
      "TE_INSERT_ROW_BELOW",
    );
    ...
    this.oApp.registerBrowserEvent(
      this.elInputRadioBGColor,
      "click",
      "DRAW_QE_RADIO_OPTION",
      [2],
    );
    this.oApp.registerBrowserEvent(
      this.elInputRadioBGImg,
      "click",
      "DRAW_QE_RADIO_OPTION",
      [3],
    );
  ...
  $ON_TABLE_QE_SELECT_TEMPLATE: function (weEvent) {
    ...
    var elSaveTarget =
      !!elCurrentTable && elCurrentTable.parentNode
        ? elCurrentTable.parentNode
        : null;
  ...
  $ON_TABLE_QE_SET_IMG: function (elSelected) {
    ...
    if (n === "0") {
      ...
      for (var j = 0, nLen = this.aSelectedCells.length; j < nLen; j++) {
        jindo
          .$Element(this.aSelectedCells[j])
          .css(
            "backgroundImage",
            "url(" +
              "http://static.se2.naver.com/static/img/" +
              sImageName +
              ")",
          );
        this.aSelectedCells[j].removeAttribute(this.TMP_BGC_ATTR);
        this.aSelectedCells[j].setAttribute(
          this.TMP_BGIMG_ATTR,
  ...
  $ON_SHOW_COMMON_QE: function () {
    if (
      jindo.$Element(this.elSelectionStartTable).hasClass(this._sSETblClass)
    ) {
      this.oApp.exec("SHOW_TABLE_QE");
    } else {
      if (
        jindo
          .$Element(this.elSelectionStartTable)
  ...
  $ON_SETUP_TABLE_QE_MODE: function (nMode) {
    ...
    var sBackgroundColor =
      this.aSelectedCells[0].getAttribute(this.TMP_BGC_ATTR) ||
      "rgb(255,255,255)";
    ...
    for (var i = 1, nLen = this.aSelectedCells.length; i < nLen; i++) {
      ...
      if (this.aSelectedCells[i]) {
        if (
          sBackgroundColor !=
          this.aSelectedCells[i].getAttribute(this.TMP_BGC_ATTR)
        ) {
          bAllMatched = false;
          break;
        }
    ...
    var sBackgroundImage =
      this.aSelectedCells[0].getAttribute(this.TMP_BGIMG_ATTR) || "";
    ...
    if (this.nQEMode === 0) {
      ...
      var nTpl = this.parseIntOr0(
        this.elSelectionStartTable.getAttribute(this.ATTR_TBL_TEMPLATE),
      );
      if (nTpl) {
      ...
      var nTpl = this.parseIntOr0(
        this.elSelectionStartTable.getAttribute(this.ATTR_REVIEW_TEMPLATE),
      );
  ...
  $ON_TE_MERGE_CELLS: function () {
    ...
    var elTable = nhn.husky.SE2M_Utils.findAncestorByTagName(
      "TABLE",
      elFirstTD,
    );
    ...
    nHeight = parseInt(
      elLastTD.style.height || elLastTD.getAttribute("height"),
      10,
    );
    nWidth = parseInt(
      elLastTD.style.width || elLastTD.getAttribute("width"),
      10,
    );
    ...
    for (i = this.htSelectionSPos.y + 1; i < this.htSelectionEPos.y + 1; i++) {
      ...
      elLastTD = curTD;
      nHeight += parseInt(
        curTD.style.height || curTD.getAttribute("height"),
        10,
      );
    ...
    elFirstTD.setAttribute(
      "colSpan",
      this.htSelectionEPos.x - this.htSelectionSPos.x + 1,
    );
    elFirstTD.setAttribute(
      "rowSpan",
      this.htSelectionEPos.y - this.htSelectionSPos.y + 1,
    );
    ...
    for (i = 1; i < this.aSelectedCells.length; i++) {
      ...
      if (elTD.parentNode) {
        ...
        if (
          htBrowser.ie &&
          (htBrowser.nativeVersion == 9 || htBrowser.nativeVersion == 10) &&
          (htBrowser.version == 9 || htBrowser.version == 10)
        ) {
  ...
  $ON_TE_SPLIT_COLUMN: function () {
    ...
    for (var i = 0, nLen = this.aSelectedCells.length; i < nLen; i++) {
      ...
      if (
        htBrowser.ie &&
        (htBrowser.nativeVersion >= 9 || htBrowser.nativeVersion <= 11) &&
        (htBrowser.version >= 9 || htBrowser.version <= 11)
      ) {
  ...
  $ON_TE_SPLIT_ROW: function () {
    ...
    for (var i = 0, nLen = this.aSelectedCells.length; i < nLen; i++) {
      ...
      elNextTRInsertionPoint.parentNode.insertBefore(
        elNewTR,
        elNextTRInsertionPoint.nextSibling,
      );
      nNewRows++;
  ...
  $ON_EVENT_EDITING_AREA_MOUSEDOWN: function (wevE) {
    ...
    switch (this.nStatus) {
      case this.STATUS.S_0:
        ...
        var elTD = nhn.husky.SE2M_Utils.findClosestAncestorAmongTagNames(
          this._aCellName,
          wevE.element,
        );
        ...
        if (elTD && this._rxCellNames.test(elTD.tagName)) {
          ...
          if (
            ...
            !jindo.$Element(elTBL).hasClass(this._sSEReviewTblClass)
          ) {
            return;
          }
  ...
  _coverResizeLayer: function () {
    ...
    this.elResizeCover.style.width =
      size.width - this.nPageLeftRightMargin + "px";
    this.elResizeCover.style.height =
      size.height - this.nPageTopBottomMargin + "px";
  ...
  _reassignCellSizes: function (elTable) {
    ...
    var aAllCellsWithSizeInfo = new Array(
      allCells[0].length + allCells[1].length,
    );
    var numCells = 0;
    ...
    for (var n = 0; n < 2; n++) {
      for (var i = 0; i < allCells[n].length; i++) {
        ...
        if (elCell.getComputedStyle) {
          ...
          nPaddingBottom = parseFloat(
            getComputedStyle(elCell).paddingBottom,
            10,
          );

          // 최초 리사이징 직전 width attribute에서 style의 width로 이행하는 과정에서 미세보정이 있기 때문에 크기가 조금 변한다.
          nBorderLeft = parseFloat(
            getComputedStyle(elCell).borderLeftWidth,
            10,
          );
          nBorderRight = parseFloat(
            getComputedStyle(elCell).borderRightWidth,
            10,
          );
          nBorderTop = parseFloat(getComputedStyle(elCell).borderTopWidth, 10);
          nBorderBottom = parseFloat(
            getComputedStyle(elCell).borderBottomWidth,
            10,
          );
        } else {
          // 이 방식은 inherit된 스타일을 가져오지 못하는 문제와 함께, 일부 브라우저의 소수점 값을 버림하는 문제가 있다.
          ...
          nBorderLeft = this.parseBorder(
            welCell.css("borderLeftWidth"),
            welCell.css("borderLeftStyle"),
          );
          nBorderRight = this.parseBorder(
            welCell.css("borderRightWidth"),
            welCell.css("borderRightStyle"),
          );
          nBorderTop = this.parseBorder(
            welCell.css("borderTopWidth"),
            welCell.css("borderTopStyle"),
          );
          nBorderBottom = this.parseBorder(
            welCell.css("borderBottomWidth"),
            welCell.css("borderBottomStyle"),
          );
        }
        ...
        if (!nWidth && !nHeight) {
          ...
          nOffsetWidth =
            elCell.offsetWidth -
            (nPaddingLeft + nPaddingRight + nBorderLeft + nBorderRight) +
            "px";
          nOffsetHeight =
            elCell.offsetHeight -
            (nPaddingTop + nPaddingBottom + nBorderTop + nBorderBottom) +
            "px";
        }
        ...
        aAllCellsWithSizeInfo[numCells++] = [
          elCell,
          nOffsetWidth,
          nOffsetHeight,
        ];
  ...
  _mousedown_ResizeCover: function (oEvent) {
    ...
    var nOffsetX =
      (parseInt(this.htResizing.elCell.getAttribute("colspan")) || 1) - 1;
    var nOffsetY =
      (parseInt(this.htResizing.elCell.getAttribute("rowspan")) || 1) - 1;
    ...
    this.htAllAffectedCells = this._getAllAffectedCells(
      x,
      y,
      this.htResizing.nResizeMode,
  ...
  _mousemove_ResizeCover: function (oEvent) {
    ...
    if (
      ...
      if (
        this.htResizing.nPreviousResizeMode != undefined &&
    ...
    if (this.htResizing.nResizeMode == 1) {
      this.elResizeGrid.style.left =
        oEvent.pos().clientX -
        this.nClientXDiff -
        this.parseIntOr0(this.elResizeGrid.style.width) / 2 +
        "px";
    } else {
      this.elResizeGrid.style.top =
        oEvent.pos().clientY -
        this.nClientYDiff -
        this.parseIntOr0(this.elResizeGrid.style.height) / 2 +
        "px";
    }
    // --[SMARTEDITORSUS-1504]
  },
  ...
  _mouseup_ResizeCover: function (oEvent) {
    ...
    if (this.htResizing.nResizeMode == 1) {
      ...
      if (
        this.htAllAffectedCells.nMinBefore != -1 &&
        nWidthChange < -1 * this.htAllAffectedCells.nMinBefore
      ) {
        nWidthChange =
          -1 * this.htAllAffectedCells.nMinBefore + this.MIN_CELL_WIDTH;
      }
      if (
        this.htAllAffectedCells.nMinAfter != -1 &&
        nWidthChange > this.htAllAffectedCells.nMinAfter
      ) {
  ...
  _isOnBorder: function (wevE) {
    ...
    this.htResizing.elTable = nhn.husky.SE2M_Utils.findAncestorByTagName(
      "TABLE",
    ...
    if (
      !jindo.$Element(this.htResizing.elTable).hasClass(this._sSETblClass) &&
    ...
    this.htResizing.nBorderSize = this.parseIntOr0(
      this.htResizing.elTable.border,
    );
    ...
    if (nBottomBorderCriteria <= nAdjustedDraggableCellEdge2) {
      this.htResizing.nBorderTopPos =
        this.htResizing.elCell.offsetHeight + nAdjustedDraggableCellEdge1 - 1;
      this.htResizing.nResizeMode = 2;
    }
    // --[SMARTEDITORSUS-1504]
    ...
    if (this.htResizing.htEPos.offsetX <= nAdjustedDraggableCellEdge1) {
      ...
      if (
        this.htResizing.elTable.scrollLeft != this.htResizing.elCell.offsetLeft
      ) {
        this.htResizing.nHA = -1;
    ...
    if (nRightBorderCriteria <= nAdjustedDraggableCellEdge1) {
      this.htResizing.nBorderLeftPos =
        this.htResizing.elCell.offsetWidth + nAdjustedDraggableCellEdge1 - 1;
  ...
  _showCellResizeGrip: function () {
    ...
    if (this.htResizing.nResizeMode == 1) {
      ...
      this._setResizerSize(
        ...
        this.parseIntOr0(jindo.$Element(this.elIFrame).css("height")),
      );
      this.elResizeGrid.style.top = "0px";
      this.elResizeGrid.style.left =
        this.htResizing.elCell.clientWidth +
      ...
      this.elResizeGrid.style.top =
        this.htResizing.elCell.clientHeight +
  ...
  _getAllAffectedCells: function (basis_x, basis_y, iResizeMode, oTable) {
    ...
    if (iResizeMode == 1) {
      for (var y = 0; y < iTblY; y++) {
        if (
          aCellsBefore.length > 0 &&
          aCellsBefore[aCellsBefore.length - 1] == oTbl[basis_x][y]
        ) {
          continue;
        }
      ...
      if (oTbl.length > basis_x + 1) {
        for (var y = 0; y < iTblY; y++) {
          if (
            aCellsAfter.length > 0 &&
            aCellsAfter[aCellsAfter.length - 1] == oTbl[basis_x + 1][y]
          ) {
            continue;
          }
      ...
      for (var x = 0; x < iTblX; x++) {
        if (
          aCellsBefore.length > 0 &&
          aCellsBefore[aCellsBefore.length - 1] == oTbl[x][basis_y]
        ) {
          continue;
        }
  ...
  _createCellResizeGrip: function () {
    ...
    try {
      this.elTmp.innerHTML =
        '<div style="position:absolute; overflow:hidden; z-index: 99; "><div onmousedown="return false" style="background-color:#000000;filter:alpha(opacity=0);opacity:0.0;-moz-opacity:0.0;-khtml-opacity:0.0;cursor: col-resize; left: 0px; top: 0px; width: 100%; height: 100%;font-size:1px;z-index: 999; "></div></div>';
    ...
    var oContainer = jindo.$$.getSingle(
  ...
  _deleteSelectedCells: function () {
    ...
    if (nSelectionWidth == nWidth) {
      for (var i = 0, nLen = aTR.length; i < nLen; i++) {
        ...
        if (
          !this.htMap[0][i] ||
          !this.htMap[0][i].parentNode ||
          this.htMap[0][i].parentNode.tagName !== "TR"
        ) {
    ...
    if (aTR.length < 1) {
      this.elSelectionStartTable.parentNode.removeChild(
  ...
  _insertColumnAfter: function () {
    ...
    for (var y = 0, nYLen = this.htMap[0].length; y < nYLen; y++) {
      ...
      for (var x = this.htSelectionEPos.x; x >= this.htSelectionSPos.x; x--) {
        ...
        elInsertionPt = null;
        for (
          var xx = this.htSelectionEPos.x;
          xx >= this.htSelectionSPos.x;
          xx--
        ) {
    ...
    for (var y = this.htSelectionSPos.y; y <= this.htSelectionEPos.y; y++) {
      ...
      if (
        ...
        for (var i = 0, len = aCellClone.length; i < len; i++) {
          ...
          if (
            this._rxCellNames.test(elCloneTD.nodeName) &&
            elPreviousTD &&
            this._rxCellNames.test(elPreviousTD.nodeName)
          ) {
            // --[SMARTEDITORSUS-1672]
            // [SMARTEDITORSUS-1742]
  ...
  _startCellSelection: function () {
    ...
    this.htEditingAreaPos.bottom =
      this.htEditingAreaPos.top + this.htEditingAreaPos.height;
    this.htEditingAreaPos.width = elIFrame.offsetWidth;
    this.htEditingAreaPos.right =
      this.htEditingAreaPos.left + this.htEditingAreaPos.width;
...
*/
    ...
    var elLastCell = null;
    for (
      var y = 0, nRealY = 0, nYLen = aCellMapping[0].length;
      y < nYLen;
      y++, nRealY++
    ) {
      elLastCell = null;
    ...
    for (var i = 0; i < this.aSelectedCells.length; i++) {
      ...
      if (elCell) {
        ...
        if (elCell.getAttribute(this.TMP_BGIMG_ATTR)) {
          welCell.css(
            "backgroundImage",
            elCell.getAttribute(this.TMP_BGIMG_ATTR),
          );
  ...
  _applyTableTemplate: function (elTable, nTemplateIdx) {
    ...
    this._doApplyTableTemplate(
      elTable,
      nhn.husky.SE2M_TableTemplate[nTemplateIdx],
      false,
    );
    ...
    if (elTable.getAttribute(this.ATTR_TBL_TEMPLATE)) {
      this._doApplyTableTemplate(
        ...
        nhn.husky.SE2M_TableTemplate[
          this.parseIntOr0(elTable.getAttribute(this.ATTR_TBL_TEMPLATE))
        ],
        true,
      );
    }
  ...
  _doApplyTableTemplate: function (elTable, htTableTemplate, bClearStyle) {
    ...
    if (ht1stRowStyle) {
      ...
      for (
        var ii = 0, nNumCells = aTR[0].childNodes.length;
        ii < nNumCells;
        ii++
      ) {
    ...
    if (aRowStyle) {
      ...
      for (var i = nStartRowNum, nLen = aTR.length; i < nLen; i++) {
        for (
          var ii = 0, nNumCells = aTR[i].childNodes.length;
          ii < nNumCells;
          ii++
        ) {
          ...
          this._copyStylesTo(
            elTmp,
            aRowStyle[(i + nStartRowNum) % nNumStyles],
            bClearStyle,
          );
        }
      }
...
*/
  ...
  _removeEmptyTextNode_IE: function (elPair) {
    ...
    if (
      elEmptyTextNode &&
      elEmptyTextNode.nodeType == 3 &&
      !/\S/.test(elEmptyTextNode.nodeValue)
    ) {
...
nhn.husky.HuskyCore.mixin(nhn.husky.SE2M_Hyperlink, {
  ...
  $ON_MSG_HYPERLINK_LAYER_SHOWN: function () {
    ...
    if (oAnchor && !this.oSelection.collapsed) {
      ...
      try {
        ...
        this.oLinkInput.value =
          sHref && sHref.indexOf("#") == -1 ? sHref : "http://";
      } catch (e) {
  ...
  _validateTarget: function () {
    ...
    if (oNavigator.ie) {
      jindo.$A(this.oSelection.getNodes(true)).forEach(function (
        elNode,
        index,
        array,
      ) {
        if (
          !!elNode &&
          elNode.nodeType == 1 &&
          elNode.tagName.toLowerCase() == "iframe" &&
          elNode.getAttribute("s_type").toLowerCase() == "db"
        ) {
          bReturn = false;
          jindo.$A.Break();
        }
    ...
    if (this._validateURL(sURL)) {
      ...
      if (this.oSelection.collapsed) {
        ...
        if (oAgent.ie && (oAgent.version === 8 || oAgent.nativeVersion === 8)) {
          // nativeVersion 으로 IE7 호환모드인 경우 확인
          this.oApp.exec("IE_FOCUS", []);
          ...
          if (this._isExceptional()) {
            ...
            jindo.$A(this.oSelection.getNodes(true)).forEach(function (
              ...
              if (value.nodeType === 3) {
                ...
                oEmptySelection.selectNode(value);
                oEmptySelection.pasteHTML(
                  sTempUrl + jindo.$Element(value).outerHTML() + "</a>",
                );
              }
            }, this);
          } else {
            this.oApp.exec("EXECCOMMAND", [
              "createLink",
              false,
              this.sATagMarker + nSession + encodeURIComponent(sURL),
              { bDontAddUndoHistory: true },
            ]);
          }
        }
          ...
          if (sHref && sHref.match(rxMarker)) {
            ...
            if (oAgent.ie) {
              ...
              if (this._validateURL(sAContent)) {
                jindo
                  .$Element(elATag)
                  .html(jindo.$Element(elATag).attr("href"));
              }
            }
            /*else{
  ...
  _isExceptional: function () {
    ...
    if (
      ...
      bImg = jindo.$A(this.oSelection.getNodes()).some(function (
        value,
        index,
        array,
      ) {
    ...
    bEmail = jindo.$A(this.oSelection.getTextNodes()).some(function (
      value,
      index,
      array,
    ) {
  ...
  _validateURL: function (sURL) {
    ...
    return /^(http|https|ftp|mailto):(\/\/)?(([-가-힣]|\w)+(?:[\/\.:@]([-가-힣]|\w)+)+)\/?(.*)?\s*$/i.test(
      sURL,
    );
  },
  //@lazyload_js]
...
nhn.husky.HuskyCore.mixin(nhn.husky.SE2M_LineHeightWithLayerUI, {
  ...
  _assignHTMLObjects: function (elAppContainer) {
    ...
    this.aLIOptions = jindo.$A(
      jindo.$$("UL.se2_l_line_height > LI", this.oDropdownLayer),
    )._array;

    this.oInput = jindo.$$.getSingle("INPUT", this.oDropdownLayer);

    var tmp = jindo.$$.getSingle(
  ...
  $LOCAL_BEFORE_FIRST: function () {
    ...
    for (var i = 0; i < this.aLIOptions.length; i++) {
      this.oApp.registerBrowserEvent(
        this.aLIOptions[i],
        "click",
        "SET_LINEHEIGHT_FROM_LAYER_UI",
        [this._getLineHeightFromLI(this.aLIOptions[i])],
      );
    }
  ...
  $ON_SE2M_TOGGLE_LINEHEIGHT_LAYER: function () {
    this.oApp.exec("TOGGLE_TOOLBAR_ACTIVE_LAYER", [
      this.oDropdownLayer,
      null,
      "LINEHEIGHT_LAYER_SHOWN",
      [],
      "LINEHEIGHT_LAYER_HIDDEN",
      [],
    ]);
  ...
  $ON_LINEHEIGHT_LAYER_SHOWN: function () {
    ...
    if (nLineHeight != null && nLineHeight !== 0) {
      ...
      var elBtn = elLi ? this._getLineHeightButtonFromLI(elLi) : null;
      if (elBtn) {
        jindo.$Element(elBtn).addClass("active");
      }
    } else {
      this.oInput.value = "";
    }
  ...
  $ON_SE2M_SET_LINEHEIGHT_FROM_DIRECT_INPUT: function () {
    ...
    var sValue =
      nInputValue < this.MIN_LINE_HEIGHT ? this.MIN_LINE_HEIGHT : nInputValue;
  ...
  _getLineHeightButtonFromLI: function (elLi) {
    var n;
    if (!elLi) {
      return null;
    }
  ...
  _getLineHeightFromLI: function (elLi) {
    var btn, n;
    btn = this._getLineHeightButtonFromLI(elLi);
    if (!btn) {
      return "";
    }
    n = btn.firstChild;
  ...
  _clearOptionSelection: function (elLi) {
    var btn, i;
    for (i = 0; i < this.aLIOptions.length; i++) {
      btn = this._getLineHeightButtonFromLI(this.aLIOptions[i]);
      if (btn) {
        jindo.$Element(btn).removeClass("active");
      }
    }
  },
  //@lazyload_js]
...
nhn.husky.HuskyCore.mixin(nhn.husky.SE2M_QuickEditor_Common, {
  ...
  setOpenType: function (sType, bBol) {
    ...
    if (
      typeof this._environmentData == "undefined" ||
      this._environmentData == null
    ) {
      this._environmentData = {};
    }
    if (
      typeof this._environmentData[sType] == "undefined" ||
      this._environmentData[sType] == null
    ) {
      this._environmentData[sType] = {};
    }
    if (
      typeof this._environmentData[sType].isOpen == "undefined" ||
      this._environmentData[sType].isOpen == null
    ) {
  ...
  $LOCAL_BEFORE_FIRST: function (sMsg) {
    ...
    this.woStandard = jindo
      .$Element(this.oApp.htOptions.elAppContainer)
      .offset();
    this._qe_wrap = jindo.$$.getSingle(
      "DIV.quick_wrap",
      this.oApp.htOptions.elAppContainer,
    );
    ...
    new jindo.DragArea(this._qe_wrap, {
      ...
      dragEnd: function (oCustomEvent) {
        ...
        that._environmentData[that._currentType].position = [
          richEle.css("top"),
          richEle.css("left"),
        ];
        //}
...
*/
  ...
  layer_hide: function (weEvent) {
    ...
    jindo
      .$Element(
        jindo.$$.getSingle(
          "._" + this._environmentData[this._currentType].type,
          this.currentEle,
        ),
      )
      .hide();
...
nhn.FindReplace = jindo.$Class({
    ...
    if (this.document.body.createTextRange) {
      try {
        ...
        if (this.document.selection) {
          // document.selection 이 있으면 selection 에서 TextRange 생성
          this._range = this.document.selection.createRangeCollection().item(0);
          this._range.collapse(false);
        } else if (!this._range) {
          // [SMARTEDITORSUS-1528] IE11인 경우 createTextRange 로 TextRange 생성
          this._range = this.document.body.createTextRange();
        } else {
          // [SMARTEDITORSUS-1837] 이미 생성되어 있는 TextRange를 이용해 collapseEnd 하면 다음 문자를 찾을 수 있다.
          this._range.collapse(false);
        }
  ...
  _replace: function (
    ...
    bCaseMatch = bCaseMatch || false;
    var bMatch,
      selectedText = oSelection.toString();
    if (bCaseMatch) bMatch = selectedText == sOriginalWord;
    else bMatch = selectedText.toLowerCase() == sOriginalWord.toLowerCase();
  ...
  _getNextTextNode: function (elNode, bDisconnected) {
    ...
    while (true) {
      ...
      if (
        elNode &&
        elNode.nodeType != 3 &&
        !this.rxInlineContainer.test(elNode.tagName)
      ) {
        bDisconnected = true;
      }
      ...
      for (var i = 0, niLen = aTexts.length; i < niLen; i++) {
        ...
        for (var j = sText.length - nOriginLen; j >= 0; j--) {
          ...
          if (
            bWholeWord &&
            j > 0 &&
            sText.charAt(j - 1).match(/[a-zA-Z가-힣]/)
          ) {
            continue;
          }
...
nhn.DraggableLayer = jindo.$Class({
  $init: function (elLayer, oOptions) {
    ...
    var nTop =
      this.toInt(jindo.$Element(elLayer).offset().top) - this.aBasePosition.top;
    var nLeft =
      this.toInt(jindo.$Element(elLayer).offset().left) -
      this.aBasePosition.left;
    ...
    this.$FnMouseDown = jindo.$Fn(
      jindo.$Fn(this._mousedown, this).bind(elLayer),
      this,
    );
    this.$FnMouseMove = jindo.$Fn(
      jindo.$Fn(this._mousemove, this).bind(elLayer),
      this,
    );
    this.$FnMouseUp = jindo.$Fn(
      jindo.$Fn(this._mouseup, this).bind(elLayer),
      this,
    );
  ...
  _mousedown: function (elLayer, oEvent) {
    ...
    this.MouseOffsetY =
      oEvent.pos().clientY -
      this.toInt(elLayer.style.top) -
      this.aBasePosition["top"];
    this.MouseOffsetX =
      oEvent.pos().clientX -
      this.toInt(elLayer.style.left) -
      this.aBasePosition["left"];
  ...
  _mousemove: function (elLayer, oEvent) {
    var nTop =
      oEvent.pos().clientY - this.MouseOffsetY - this.aBasePosition["top"];
    var nLeft =
      oEvent.pos().clientX - this.MouseOffsetX - this.aBasePosition["left"];
